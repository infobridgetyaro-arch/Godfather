...
// True daily quota exhaustion — clears at midnight Pacific (08:00 UTC).
const exhaustedUntil = new Map<number, number>(); // index → timestamp when marked exhausted

// Temporary rate-limit backoff (HTTP 429 or rateLimitExceeded / userRateLimitExceeded).
// These are NOT daily exhaustion — YouTube is just asking us to slow down.
// Each entry auto-expires after RATE_LIMIT_BACKOFF_MS without a server restart.
const rateLimitedUntil = new Map<number, number>(); // index → timestamp when backoff expires
const RATE_LIMIT_BACKOFF_MS = 5 * 60_000; // 5 min — try again after a short pause
...
/**
 * Returns true if key `idx` is in a temporary rate-limit backoff window.
 * Auto-expires the entry once the window has passed.
 */
function isRateLimited(idx: number): boolean {
  const until = rateLimitedUntil.get(idx);
  if (until === undefined) return false;
  if (Date.now() >= until) {
    rateLimitedUntil.delete(idx);
    logger.info({ keyIndex: idx + 1 }, "[youtube] Rate-limit backoff expired — key available again");
    return false;
  }
  return true;
}
...
/**
 * Mark current key as unavailable and rotate to the next available one.
 * @param temporary  true → short backoff (429 / rate-limit); false → daily quota exhausted
 * Returns true if a fresh key is now active, false if all keys are currently unavailable.
 */
function rotateApiKey(temporary: boolean): boolean {
  if (temporary) {
    const backoffUntil = Date.now() + RATE_LIMIT_BACKOFF_MS;
    rateLimitedUntil.set(currentKeyIndex, backoffUntil);
    const backoffMin = Math.round(RATE_LIMIT_BACKOFF_MS / 60_000);
    logger.warn(
      { keyIndex: currentKeyIndex + 1, totalKeys: apiKeys.length, backoffMin },
      `[youtube] API key #${currentKeyIndex + 1} temporarily rate-limited — backing off ${backoffMin} min`
    );
  } else {
    exhaustedUntil.set(currentKeyIndex, Date.now());
    logger.warn(
      { exhaustedKey: currentKeyIndex + 1, totalKeys: apiKeys.length },
      `[youtube] API key #${currentKeyIndex + 1} daily quota exhausted — rotating to next key`
    );
  }
  for (let i = 1; i <= apiKeys.length; i++) {
    const next = (currentKeyIndex + i) % apiKeys.length;
    if (!exhaustedUntil.has(next) && !isRateLimited(next)) {
  ...
  const anyTempOnly = apiKeys.some((_, i) => !exhaustedUntil.has(i) && isRateLimited(i));
  if (anyTempOnly) {
    logger.warn(
      { totalKeys: apiKeys.length, backoffMin: Math.round(RATE_LIMIT_BACKOFF_MS / 60_000) },
      `[youtube] All keys temporarily rate-limited — will auto-recover in ~${Math.round(RATE_LIMIT_BACKOFF_MS / 60_000)} min`
    );
  } else {
    logger.error(
      { totalKeys: apiKeys.length },
      `[youtube] All ${apiKeys.length} API key(s) daily quota exhausted — add more keys or wait for midnight reset`
    );
  }
  return false;
}
...
function isQuotaExhausted(): boolean {
  ...
  // A key is available if it's neither daily-exhausted nor temporarily rate-limited
  return apiKeys.every((_, idx) => exhaustedUntil.has(idx) || isRateLimited(idx));
}
...
// liveChatMessages.list costs 5 units/call; at 15 s minimum that is ≤ 1,200 units/hour,
// keeping chat + subscriber + viewer calls under ~1,500 units/hour so a single key
// lasts comfortably ≥ 3 hours of continuous streaming.
const DEFAULT_CHAT_POLL_MS = 15_000; // 15 s default (YouTube hint overrides if larger)
const MIN_CHAT_POLL_MS     = 15_000; // 15 s floor — never poll faster than this
...
// Reasons that mean the daily quota is truly used up — key is dead until midnight.
const DAILY_EXHAUSTED_REASONS = new Set([
  "quotaExceeded",
  "dailyLimitExceeded",
]);

// Reasons that mean YouTube is asking us to slow down temporarily — NOT daily exhaustion.
// These back off for RATE_LIMIT_BACKOFF_MS and then auto-recover.
const TEMP_RATE_LIMIT_REASONS = new Set([
  "rateLimitExceeded",
...
async function fetchWithKeyRotation(
  ...
  while (triedKeys.size < apiKeys.length) {
    ...
    try {
      ...
      if (res.status === 429) {
        // HTTP 429 = temporary rate limit — NOT daily quota exhaustion.
        // Back off for RATE_LIMIT_BACKOFF_MS; the key will auto-recover.
        const body: any = await res.json().catch(() => ({}));
        const reason = body?.error?.errors?.[0]?.reason ?? "rateLimitExceeded";
        logger.warn({ label, status: 429, keyIndex: currentKeyIndex, reason }, "[youtube] Temporary rate limit — backing off");
        recordApiRequest(false, `429: ${reason} (temporary)`);
        triedKeys.add(currentKeyIndex);
        const rotated = rotateApiKey(/* temporary */ true);
        if (!rotated) return null;
        continue;
      }

      if (res.status === 403) {
        // Read the body to find out WHY it's a 403 before deciding how to handle it
        const body: any = await res.json().catch(() => ({}));
        const reason: string = body?.error?.errors?.[0]?.reason ?? "unknown";
        if (DAILY_EXHAUSTED_REASONS.has(reason)) {
          // True daily quota exhaustion — key is dead until midnight reset.
          logger.warn({ label, status: 403, keyIndex: currentKeyIndex, reason }, "[youtube] Daily quota exhausted — rotating key");
          recordApiRequest(false, `403 daily quota: ${reason}`);
          triedKeys.add(currentKeyIndex);
          const rotated = rotateApiKey(/* temporary */ false);
          if (!rotated) return null;
          continue;
        } else if (TEMP_RATE_LIMIT_REASONS.has(reason)) {
          // Temporary rate limit in a 403 wrapper — back off, don't mark as daily-exhausted.
          logger.warn({ label, status: 403, keyIndex: currentKeyIndex, reason }, "[youtube] Temporary rate limit (403) — backing off");
          recordApiRequest(false, `403 rate limit: ${reason} (temporary)`);
          triedKeys.add(currentKeyIndex);
          const rotated = rotateApiKey(/* temporary */ true);
          if (!rotated) return null;
          continue;
...
export function primeLiveDetection(streamId: string, channelId: string | null | undefined): void {
  ...
  // 3 attempts: immediate, +10 s, +30 s — each search.list costs 100 quota units,
  // so limiting to 3 retries caps startup cost at 300 units instead of 600.
  const retryDelaysMs = [0, 10_000, 30_000];
  let attempt = 0;
...
export function getApiKeyStatus(): { total: number; active: number; exhausted: number[]; rateLimited: number[]; currentIndex: number } {
  // Expire stale rate-limit entries before reporting
  for (const [idx, until] of rateLimitedUntil.entries()) {
    if (Date.now() >= until) rateLimitedUntil.delete(idx);
  }
  return {
    total: apiKeys.length,
    active: currentKeyIndex,
    exhausted: Array.from(exhaustedUntil.keys()),
    rateLimited: Array.from(rateLimitedUntil.keys()),
...
export function recordApiRequest(success: boolean, errorMsg?: string) {
  ...
  if (success) {
    t.lastSuccessAt = now;
    // If this key was previously marked exhausted or rate-limited but a call
    // just succeeded, clear both states immediately.
    ...
    if (rateLimitedUntil.has(idx)) {
      rateLimitedUntil.delete(idx);
      eventLog.push({ ts: now, type: "success", keyIndex: idx, msg: `Key #${idx + 1}: rate-limit backoff cleared on successful response` });
      logger.info({ keyIndex: idx + 1 }, "[youtube] Rate-limit backoff auto-cleared after successful API response");
    }
    if (!exhaustedUntil.has(idx) && !rateLimitedUntil.has(idx) && t.totalRequests % 50 === 1) {
...
/**
 * Manually clear exhausted AND rate-limited states for one or all keys.
 * Useful when keys were wrongly marked (e.g. a false-positive rate-limit)
 * or when the operator wants to force-activate a key early.
 */
export function clearKeyExhaustion(targetIndex?: number): { cleared: number[] } {
  const cleared: number[] = [];
  if (targetIndex === undefined) {
    const dailyKeys  = Array.from(exhaustedUntil.keys());
    const limitKeys  = Array.from(rateLimitedUntil.keys());
    exhaustedUntil.clear();
    rateLimitedUntil.clear();
    const all = Array.from(new Set([...dailyKeys, ...limitKeys]));
    cleared.push(...all);
    logger.info({ cleared }, "[youtube] All key exhaustion and rate-limit states cleared manually");
  } else {
    if (exhaustedUntil.has(targetIndex)) {
      exhaustedUntil.delete(targetIndex);
      cleared.push(targetIndex);
    }
    if (rateLimitedUntil.has(targetIndex)) {
      rateLimitedUntil.delete(targetIndex);
      if (!cleared.includes(targetIndex)) cleared.push(targetIndex);
    }
    if (cleared.length > 0) {
      logger.info({ keyIndex: targetIndex + 1 }, `[youtube] Key #${targetIndex + 1} exhaustion/rate-limit cleared manually`);
    }
  }
  // Re-select first available key
  const firstAvail = apiKeys.findIndex((_, i) => !exhaustedUntil.has(i) && !isRateLimited(i));
...
export function getDetailedApiStatus() {
  const now = Date.now();
  // Expire stale rate-limit entries before computing status
  for (const [idx, until] of rateLimitedUntil.entries()) {
    if (now >= until) rateLimitedUntil.delete(idx);
  }

  // Quota reset = midnight Pacific (08:00 UTC)
  const nextReset = new Date();
  nextReset.setUTCHours(8, 0, 0, 0);
  ...
  const keys = apiKeys.map((_, idx) => {
    ...
    const recentReqs = t
      ...
      : 0;
    const isDailyExhausted = exhaustedUntil.has(idx);
    const rlUntil          = rateLimitedUntil.get(idx) ?? null;
    const isTempLimited    = rlUntil !== null && now < rlUntil;
    // resetAt: for daily exhaustion show midnight; for temp rate-limit show backoff expiry
    const resetAt = isDailyExhausted ? quotaResetAt : (isTempLimited ? rlUntil : null);
    return {
      index: idx,
      masked: maskApiKey(apiKeys[idx]),
      isActive: idx === currentKeyIndex && !isDailyExhausted && !isTempLimited,
      isExhausted: isDailyExhausted,
      isRateLimited: isTempLimited,
      rateLimitedUntil: isTempLimited ? rlUntil : null,
  ...
  const allDailyExhausted = apiKeys.length > 0 && apiKeys.every((_, i) => exhaustedUntil.has(i));
  const allUnavailable    = apiKeys.length > 0 && isQuotaExhausted();
  // allRateLimited: all keys unavailable but NONE are daily-exhausted → temp issue, recovers soon
  const allRateLimited    = allUnavailable && !allDailyExhausted;

  return {
    totalKeys: apiKeys.length,
    activeKeyIndex: currentKeyIndex,
    /** @deprecated use allUnavailable — kept for backward compat with existing frontend reads */
    allExhausted: allUnavailable,
    allDailyExhausted,
    allRateLimited,
    allUnavailable,
    quotaResetAt,
