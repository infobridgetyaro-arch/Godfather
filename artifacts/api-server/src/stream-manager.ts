...
// Per-stream wall-clock timestamp of the last AQM-initiated hardKillAndRestart.
// Prevents AQM from restarting again within AQM_RESTART_MIN_INTERVAL_MS even if
// the monitoring window fires (e.g. post-reconnect CPU spike, overlay joins).
// Cleared only when the user explicitly stops the stream (stopStream).
const aqmRestartCooldown = new Map<string, number>();
const AQM_RESTART_MIN_INTERVAL_MS = 5 * 60_000; // 5 min between AQM-triggered restarts
...
// First two reconnect attempts are near-instant so users never notice a gap.
// Subsequent attempts back off exponentially to avoid hammering broken sources.
const BACKOFF_DELAYS_MS = [200, 500, 5_000, 15_000, 30_000, 60_000];
...
function buildFFmpegArgs(
  ...
  if (isBrowserCamera) {
    ...
    // SourceRelay normalizes each yt-dlp session to MPEG-TS before this input.
    ...
    // SourceRelay remuxes each Streamlink FLV session to MPEG-TS before this
    // input. The main demuxer therefore never sees concatenated FLV headers.
    //
    ...
    args.push(
      ...
      "-analyzeduration", "0",
      "-probesize", "32768",
      // The per-session normalizer already absorbs container/probe jitter.
      // Keep the main decoder queue bounded so encoder stalls cannot create a
      // large compressed-packet backlog.
    "-thread_queue_size", "256",
      "-fflags", "+discardcorrupt+genpts",
      "-flags", "low_delay",
      "-i", "pipe:0",
    ...
    args.push(
      ...
      // 1024 packets: raised from 256 after live logs showed "[in#0/mpegts]
      // Thread message queue blocking" at the moment a Facebook reconnect
      // delivered its first burst of MPEG-TS data.  After a reconnect the
      // encoder sits idle (CPU ~5%, queues drained); the new Facebook data
      // arrives as a burst that fills the 256-packet queue in milliseconds,
      // blocking the demuxer thread and cascading into speed=0.2x.  1024
      // provides ~4× more headroom to absorb the reconnect surge.  Memory
      // impact is bounded: Facebook MPEG-TS TS packets are 188 bytes, so
      // 1024 packets ≈ 192 KB — negligible compared to the rawvideo queues.
  ...
  // thread_queue_size=16: at 2fps each frame is ~3.7–8MB; 16 frames = 60–130MB
  // max queue — well within budget for a streaming server and large enough to
  // absorb short render-thread stalls without blocking FFmpeg's demuxer thread.
  // Previously set to 4 (15–33MB) which caused frequent "Thread message queue
  // blocking" warnings and stalled the encoder pipeline even at low CPU — the
  // demuxer blocked waiting for queue space and the whole pipeline slowed below
  // 0.85x.  Raised to 16; OOM risk only arises at queue≥256 (≥1 GB).
  ...
  //   thread_queue_size=32: raised from 8 after live logs showed
  //   "[in#4/rawvideo] Thread message queue blocking" every time a Facebook
  //   source reconnected.  At reconnect the encoder CPU drops to ~5% (no
  //   source data), draining all input queues.  When the new Facebook data
  //   arrives it floods ALL inputs simultaneously — at queue=8 the UI overlay
  //   pipe fills in ~530 ms (8 × 66 ms/frame), blocking the entire filter
  //   graph and cascading into speed=0.2x → health degradation → cuts.
  //   32 × ~8 MB = ~256 MB max; at 15fps ≈ 2 s of buffer to absorb the
  //   reconnect surge.  uiRenderer backpressure (drain timeout = 4×intervalMs)
  //   still drops frames well before 32 accumulate under sustained CPU load.
  args.push(
    "-f", "rawvideo",
    "-pix_fmt", "rgba",
    "-video_size", `${scaleW}x${scaleH}`,
  ...
  if (isLocalCamera) {
    ...
    audioFilter = [
      ...
      // needed here.
      // aresample=48000:async=1000 — resample to 48 kHz AND apply async drift
      // compensation with a 1000-sample (~21 ms) correction window.  Without async,
      // cumulative PTS drift in long-running Facebook DASH sessions causes the audio
      // clock to diverge from the video clock; the muxer then starts logging
      // "non monotonous DTS" errors and discarding audio packets, which shows up as
      // periodic audio dropouts after several minutes.  async=1000 absorbs short
      // timing jitter without audible pitch-shifting artefacts.
      `[0:a]aresample=48000:async=1000[_fb_a_rs]`,
      // silence first → anullsrc drives the output timeline so DASH segment gaps
      // never stall the muxer.  dropout_transition=8: Facebook DASH manifest
      // refreshes can pause audio for 3–6 s (probe + CDN round-trip); a 2 s window
      // was too short and caused audio to permanently drop after a refresh cycle
      // because amix's fill-silence budget was exhausted before audio resumed.
      // 8 s covers the worst-case manifest refresh without spanning multiple cycles.
      `[2:a][_fb_a_rs]amix=inputs=2:duration=first:dropout_transition=8:normalize=0[_srcRaw]`,
      `[6:a]aformat=sample_fmts=fltp:channel_layouts=stereo[_vol]`,
  ...
  args.push(
    ...
    "-level", "4.1",
    "-bf", "0",
    // force-cfr=1 removed: -fps_mode cfr (below) already enforces CFR at the
    // muxer level. The x264-internal flag was redundant and caused the encoder to
    // insert an extra pass of NAL filler padding on every frame, wasting ~2% CPU.
  ...
  // rtmp_buffer=16000 (16 s): doubled from 8 s so that Facebook/Instagram RTMP
  // ingest brief hiccups (typically < 10 s) are absorbed entirely inside FFmpeg
  // without the output TCP connection ever seeing a pause. A 16 s client-side
  // buffer means network blips up to ~15 s resolve transparently — no kill,
  // no reconnect, no stream-end event on the platform side.
  // rw_timeout=30 s: give the ingest server 30 s to accept/reconnect before
  // FFmpeg declares the write dead and exits (triggering handleProcessExit at
  // 200 ms). Previously 20 s — raised so a momentarily overloaded ingest
  // doesn't trigger a false failure during peak hours.
  const teeOutputs = outputs
    .map((o) => `[f=flv:flvflags=no_duration_filesize:rtmp_live=1:rtmp_buffer=16000:rw_timeout=30000000]${o}`)
    .join("|");
...
// 45 s: Facebook/Instagram allow ~2 min grace; 45 s gives FFmpeg time to
// absorb brief source gaps (TikTok HLS segment boundaries, CPU spikes) via the
// 16-second RTMP output buffer before we consider the pipeline truly stalled.
const STALL_TIMEOUT_MS = 45_000;
const HEALTH_WARN_MS = 30_000; // warn before stall watchdog fires
...
export async function startStream(streamId: string, reuseUrl = false, keepStatus = false) {
  ...
  try {
    ...
    if (stream.youtubeStreamKey) {
      ...
      sendLog(streamId, `Output: YouTube (channel 1)`);
    }
    if ((stream as any).youtubeStreamKey2) {
      outputs.push(`rtmp://a.rtmp.youtube.com/live2/${(stream as any).youtubeStreamKey2}`);
      sendLog(streamId, `Output: YouTube (channel 2)`);
    }
    ...
    ffmpegProc.stderr?.on("data", (errData: Buffer) => {
      ...
      lines.forEach((line) => {
              ...
              if (currentProc) {
                ...
                if (aqm) {
                  ...
                  if (aqmDecision.shouldRestart) {
                    ...
                    // Cooldown guard — prevent AQM from restarting again within
                    // AQM_RESTART_MIN_INTERVAL_MS even if the window fires during
                    // the startup ramp-up phase or right after a feature/overlay joins.
                    const lastAqmRestart = aqmRestartCooldown.get(streamId) ?? 0;
                    const msSinceLast = Date.now() - lastAqmRestart;
                    if (msSinceLast < AQM_RESTART_MIN_INTERVAL_MS) {
                      const waitSec = Math.ceil((AQM_RESTART_MIN_INTERVAL_MS - msSinceLast) / 1000);
                      sendLog(
                        streamId,
                        `[AQM] Restart suppressed — last AQM restart was ${Math.round(msSinceLast / 1000)}s ago ` +
                        `(min interval ${AQM_RESTART_MIN_INTERVAL_MS / 60_000}min, ${waitSec}s remaining)`,
                      );
                    } else {
                      sendLog(
                        streamId,
                      ...
                      if (liveProc?.ffmpegProcess === ffmpegProc) {
                        aqmRestartCooldown.set(streamId, Date.now());
        ...
        if (trimmed.includes("Too many failure for output")) {
          // FFmpeg has exhausted all its internal retries on an output — the
          // RTMP session is truly dead. Reconnect immediately (200 ms) so the
          // platform ingest sees a new connection as fast as possible.
          scorerRecordRtmpError(streamId);
          const proc = activeStreams.get(streamId);
          if (proc?.ffmpegProcess === ffmpegProc) {
            const nowMs = Date.now();
            const lastRtmpErr = proc.lastRtmpErrorAt ?? 0;
            if (nowMs - lastRtmpErr < 5_000) return; // debounce burst
            proc.lastRtmpErrorAt = nowMs;
            sendLog(streamId, `[ffmpeg] RTMP output permanently dropped — reconnecting instantly...`);
            hardKillAndRestart(streamId, 200);
          }
          return;
        }
        ...
        if (
          ...
          // Transient RTMP write errors — FFmpeg's 16-second client-side output
          // buffer (rtmp_buffer=16000) absorbs most hiccups transparently. When
          // the buffer is finally exhausted, FFmpeg either recovers on its own
          // (rtmp_buffer drains the backlog) or exits, which handleProcessExit
          // picks up for an instant 200 ms reconnect.
          //
          // DO NOT call hardKillAndRestart here for Facebook/Instagram streams:
          // killing FFmpeg creates a brand-new RTMP TCP connection. Facebook and
          // Instagram treat any TCP break as stream-end, immediately showing the
          // "Your live video has ended" screen to viewers.  Instead, let FFmpeg
          // manage the output internally through its buffer/timeout, and only
          // reconnect if it genuinely gives up (handled above by "Too many failure").
          //
          // Extended debounce (30 s): only force-restart if errors persist beyond
          // what the 16-second buffer can absorb, indicating a sustained outage.
          scorerRecordRtmpError(streamId);
          const proc = activeStreams.get(streamId);
          if (proc?.ffmpegProcess === ffmpegProc) {
            const nowMs = Date.now();
            const lastRtmpErr = proc.lastRtmpErrorAt ?? 0;
            if (nowMs - lastRtmpErr < 30_000) {
              // Within the 30 s window — log once and let FFmpeg handle it
              if (nowMs - lastRtmpErr < 2_000) return; // suppress burst duplicates
              sendLog(streamId, `[ffmpeg] RTMP hiccup detected — FFmpeg buffer absorbing (no restart needed)...`);
              return;
            }
            // 30+ seconds of sustained errors: buffer exhausted, force reconnect
            proc.lastRtmpErrorAt = nowMs;
            sendLog(streamId, `[ffmpeg] RTMP sustained failure (>30s) — force reconnecting...`);
            hardKillAndRestart(streamId, 200);
          }
          return;
        }
...
function hardKillAndRestart(streamId: string, delayMs: number, forceNewUrl = false, keepStatus = false) {
  ...
  if (!keepStatus) {
    ...
    // Emit the countdown so the UI can display a live ms timer.
    broadcastStream(streamId, "reconnect_countdown", { remainingMs: delay });
  }
...
function handleProcessExit(streamId: string, code: number | null) {
  ...
  const backoffCount = restartBackoff.get(streamId) ?? 0;
  bumpBackoff(streamId);
  const delay = backoffCount < 2 ? 200 : getBackoffDelay(streamId);
  if (delay >= 1000) {
    sendLog(streamId, `[ffmpeg] Stream stopped (${reason}) — reconnecting in ${Math.round(delay / 1000)}s...`);
  } else {
    sendLog(streamId, `[ffmpeg] Stream stopped (${reason}) — reconnecting instantly...`);
  }
  sendStatus(streamId, "reconnecting");
  setTimeout(() => {
    restartScheduled.delete(streamId);
...
export function stopStream(streamId: string, keepAqmState = false) {
  ...
  memRestartCooldown.delete(streamId);
  aqmRestartCooldown.delete(streamId);
