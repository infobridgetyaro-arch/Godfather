 ...
 *
 * Maintains a data pipe from a live page URL (TikTok / YouTube / Facebook / links)
 * to FFmpeg's stdin.
 *
 * ─── KEY PRINCIPLE ──────────────────────────────────────────────────────────
 ...
 *
 * ─── LIFECYCLE ──────────────────────────────────────────────────────────────
 *  start()  → detect streamlink version → spawn the source process
 *           → remux each source session to MPEG-TS before the main encoder
 *           → pipe clean data via .on('data') + .write()
 ...
 * Streamlink's TikTok output is FLV. Appending a fresh FLV session to the same
 * long-lived FFmpeg stdin corrupts the demuxer ("Packet mismatch", "Concatenated
 * FLV") and can make FFmpeg queue decoded frames until the watchdog kills it.
 * Every pipe session is therefore remuxed to MPEG-TS by a small `-c copy` FFmpeg
 * child before it reaches the main encoder. A source exit closes only that
 * session's remuxer; the next session starts with a clean container boundary.
 *
 * The final write into the main FFmpeg stdin remains manual rather than
 * `.pipe()`, so a temporary source outage does not close the RTMP pipeline.
 *
 ...
 */
const BACKOFF_MS = [250, 1_000, 2_000, 4_000, 8_000, 15_000, 30_000, 60_000];
 ...
 */
const MID_STREAM_STALL_MS = 20_000;
...
const NORMALIZED_PIPE_SOURCE_TYPES = new Set([
  "tiktok",
  "tiktok_pipe",
  "youtube",
  "youtube_pipe",
...
/**
 * Build the low-CPU remux command used to establish a clean MPEG-TS boundary
 * for every source session before bytes reach the main encoder.
 */
export function buildPipeNormalizerArgs(
  sourceType: string,
  inputFormat?: "flv" | "mpegts" | "auto",
): string[] {
  const inputIsFlv =
    (inputFormat ?? (sourceType === "tiktok" || sourceType === "tiktok_pipe" ? "flv" : "auto")) === "flv";
  const isFacebook = sourceType === "facebook" || sourceType === "facebook_pipe";
  const analyzeduration = isFacebook ? "10000000" : "1000000";
  const probesize = isFacebook ? "5242880" : "1048576";
  return [
    "-hide_banner",
    "-loglevel", "warning",
    "-nostats",
    ...
    "-probesize", probesize,
    "-thread_queue_size", isFacebook ? "256" : "64",
    // +genpts: synthesise missing PTS from DTS (required for DASH streams whose
    //   muxed output has no PTS on some packets).
    // +igndts: for Facebook DASH the DTS can be unreliable after an internal
    //   yt-dlp merge; ignore it and rely on the generated PTS instead.
    "-fflags", isFacebook ? "+discardcorrupt+genpts+igndts" : "+discardcorrupt+genpts",
    "-i", "pipe:0",
    "-map", "0:v:0?",
    "-map", "0:a:0?",
    // ── Facebook A/V sync fix ────────────────────────────────────────────────
    // Facebook DASH serves video and audio as SEPARATE tracks with DIFFERENT
    // PTS origins — video typically starts 5–15 s earlier than audio in the
    // DASH manifest's presentationTimeOffset.  yt-dlp muxes them with
    // `ffmpeg -c copy`, which copies PTS verbatim.  `make_zero` then shifts
    // both streams by the MINIMUM PTS (video's), leaving audio still
    ...
    ...(isFacebook
      ...
      : ["-c", "copy"]),
    // Reset timestamps to zero at the start of each normaliser session.
    // Without this, a reconnect spawns a new yt-dlp that starts its DASH
    // timestamps from the beginning of the video (or from the CDN segment
    // clock), while the main encoder's clock has already advanced.  The
    // large backward jump causes "timestamp discontinuity" warnings and
    // visible A/V desync in the output.  make_zero shifts every stream so
    // the first packet's DTS lands at 0, giving the main encoder a clean
...
export class SourceRelay {
  ...
  /** Source process feeding the per-session normalizer. */
  private sourceProc: ChildProcess | null = null;
  ...
  private _kill(proc: ChildProcess | null): void {
    const upstream = this.sourceProc;
    if (upstream && upstream !== proc) {
  ...
  private _facebookArgs(_formatSelector: string): { cmd: string; args: string[] } {
    ...
    return {
      ...
      args: [
        ...
        "-f", sel,
        "--hls-use-mpegts",
        // --fixup force: instructs yt-dlp to repair any detected timing
        // inconsistencies (PCR discontinuities, DTS jumps) in the MPEG-TS
        // output before piping to FFmpeg.  Facebook DASH muxing produces
        // frequent segment-boundary DTS mismatches; without this flag those
        // mismatches accumulate into non-monotonic timestamps that FFmpeg
        // logs as audio errors and eventually drops packets for.
        "--fixup", "force",
        // --concurrent-fragments 2: download video + audio DASH tracks in
        // parallel with 2 threads.  The previous value of 4 could cause track
        // interleaving races where audio fragment N+1 arrived before video
        // fragment N was fully flushed, creating PTS ordering violations in
        // the muxed MPEG-TS stream.  2 threads maintains parallel download
        // speed while giving the muxer enough headroom to serialize correctly.
        "--concurrent-fragments", "2",
        // --buffer-size 256K: larger read chunks reduce round-trip overhead
        // on Facebook's CDN (~80–120 ms RTT) and keep the muxer fed between
        // segment boundaries, preventing the audio micro-gaps that occurred
        // when 128 K chunks stalled briefly at each CDN edge handoff.
        "--buffer-size", "256K",
        // A/V sync is handled in the per-session pipe normaliser (source-relay.ts
        // buildPipeNormalizerArgs) which decodes audio and applies
        // `aresample=async=1:first_pts=0` — more reliable than passing
        // --downloader-args to yt-dlp's internal FFmpeg merger which uses
        // `-c copy` for DASH and silently ignores audio-filter flags.
        "--socket-timeout", "30",
  ...
  private _linkArgs(quality: string): { cmd: string; args: string[] } {
    ...
    // Prefer pre-muxed containers (single file with A+V already in sync) over
    // separate bestvideo+bestaudio merges.  When yt-dlp merges two separate
    // DASH tracks internally its FFmpeg step can introduce a small audio/video
    // timestamp offset that persists through the relay and shows up as A/V
    // desync in the output.  A pre-muxed mp4/HLS format has no merge step,
    // so A and V are guaranteed to share the same clock.
    const genericQualityMap: Record<string, string> = {
      best:   "best[ext=mp4][vcodec!*=none][acodec!*=none]/best[protocol^=m3u8][vcodec!*=none][acodec!*=none]/best[vcodec!*=none][acodec!*=none]/bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best",
      "720p": "best[height<=720][ext=mp4][vcodec!*=none][acodec!*=none]/best[height<=720][protocol^=m3u8][vcodec!*=none][acodec!*=none]/best[height<=720][vcodec!*=none][acodec!*=none]/bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=720]+bestaudio/best[height<=720]/best",
      "480p": "best[height<=480][ext=mp4][vcodec!*=none][acodec!*=none]/best[height<=480][protocol^=m3u8][vcodec!*=none][acodec!*=none]/best[height<=480][vcodec!*=none][acodec!*=none]/bestvideo[height<=480][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=480]+bestaudio/best[height<=480]/best",
      "360p": "best[height<=360][ext=mp4][vcodec!*=none][acodec!*=none]/best[height<=360][vcodec!*=none][acodec!*=none]/bestvideo[height<=360]+bestaudio/best[height<=360]/best",
      "240p": "best[height<=240][ext=mp4][vcodec!*=none][acodec!*=none]/best[height<=240][vcodec!*=none][acodec!*=none]/bestvideo[height<=240]+bestaudio/best[height<=240]/best",
    };
  ...
  private async _spawn(): Promise<void> {
    ...
    let sourceProc: ChildProcess;
    try {
      sourceProc = spawn(cmd, args, { stdio: ["ignore", "pipe", "pipe"] });
    } catch (e: any) {
      this._spawning = false;
      if ((e as NodeJS.ErrnoException).code === "ENOENT") {
        this._fatal(
    ...
    const thisSourceProc = sourceProc;
    let proc: ChildProcess;
    const useNormalizer = NORMALIZED_PIPE_SOURCE_TYPES.has(this.sourceType);
    if (useNormalizer) {
      const inputFormat =
        isTikTokSource && resolvedQuality !== SourceRelay.YTDLP_TIKTOK_FALLBACK
          ? "flv"
          : "auto";
      const normalizerArgs = buildPipeNormalizerArgs(this.sourceType, inputFormat);
      this._log(`[relay:${this.sourceType}] $ ffmpeg ${normalizerArgs.join(" ")}`);
      try {
        proc = spawn("ffmpeg", normalizerArgs, { stdio: ["pipe", "pipe", "pipe"] });
      } catch (e: any) {
        this._kill(sourceProc);
        this._spawning = false;
        this._warn(`[relay] Failed to launch ffmpeg source normalizer: ${e?.message ?? e}`);
        this._scheduleRetry();
        return;
      }
      this.sourceProc = sourceProc;
      // This pipe is intentionally per-session: when Streamlink/yt-dlp exits,
      // EOF closes only the normalizer's stdin and flushes that session's TS.
      proc.stdin?.on("error", () => {});
      sourceProc.stdout?.on("error", () => {});
    ...
    const thisProc = proc;
    // When the normalizer is active, proc is the FFmpeg remuxer — label its
    // stderr as "normalizer" so it is not confused with the source process.
    const procLabel = useNormalizer ? "normalizer" : cmd;
    this.proc = proc;
    ...
    proc.stderr?.on("data", (d: Buffer) => {
      ...
      // procLabel is "normalizer" when this handler is for the MPEG-TS remuxer,
      // otherwise it is the source binary name (streamlink / yt-dlp path).
      ...
      for (const line of lines) {
        ...
        if (isSignificant) {
          this._warn(`[relay:${procLabel}] ${t}`);
        } else {
          logger.debug({ streamId: this.streamId, src: procLabel }, t);
        }
    ...
    // The source process owns extraction/network errors; include its stderr in
    // the same bounded session log and make an early source error visible.
    if (sourceProc !== proc) {
      sourceProc.stderr?.on("data", (d: Buffer) => {
        const text = d.toString();
        if (stderrFull.length < 50_000) stderrFull += text;
        for (const line of text.split("\n")) {
          ...
          // ── Bot / auth check detection ─────────────────────────────────────
          // "Sign in to confirm you're not a bot" means YouTube demands cookie
          // auth for this channel on this IP.  Retrying in a tight loop wastes
          // resources and can trigger further rate-limiting.  Fail permanently
          // so the operator sees a clear message instead of a spin loop.
          if (/sign in to confirm you'?re not a bot|sign in to confirm/i.test(t)) {
            if (this.proc === thisProc && !this.stopped && !this.permanentlyFailed) {
              this.proc = null;
              this.sourceProc = null;
              this._kill(thisProc);
              this._kill(thisSourceProc);
              this._fatal(
                `[relay] YouTube requires cookie authentication for this channel on this IP.\n` +
                `  Add a cookies.txt file (export from your browser while signed into YouTube)\n` +
                `  and configure it in Settings → YouTube Cookies, then restart the stream.`,
              );
            }
          }
        }
      });
      sourceProc.on("error", (err: NodeJS.ErrnoException) => {
        if (this.sourceProc !== thisSourceProc || this.stopped) return;
        if (err.code === "ENOENT") {
          this.proc = null;
          this.sourceProc = null;
          this._kill(thisProc);
        ...
        this._warn(`[relay:${cmd}] Source process error: ${err.message}`);
      });
      sourceProc.on("exit", (code) => {
        sourceExitCode = code;
      });
    }
    ...
    proc.on("error", (err: NodeJS.ErrnoException) => {
      ...
      this.proc = null;
      if (this.sourceProc === thisSourceProc) {
        this.sourceProc = null;
        this._kill(thisSourceProc);
      }
      if (this._currentProc === thisProc) {
    ...
    proc.on("exit", (code, signal) => {
      ...
      if (this.sourceProc === thisSourceProc) {
        this.sourceProc = null;
        // If the normalizer exits unexpectedly, do not leave its upstream
        // source process alive to write into a future reconnect session.
        this._kill(thisSourceProc);
      }
      ...
      if (isConfigError(code, stderrFull) || isConfigError(sourceExitCode, stderrFull)) {
      ...
      // ── Finite video URL: clean exit means the video finished ────────────────
      // For link_pipe sources, yt-dlp exits with code 0 when it has downloaded
      // and piped the full video. A code-0 exit after data flowed is NOT a
      // failure — it means the clip is done. Scheduling a retry here would
      // restart yt-dlp from the beginning and create an endless loop.
      // Signal-terminated exits (SIGKILL from OOM guard, etc.) still fall
      // through to the normal retry path below because those are genuine
      // interruptions, not natural completions.
      if (this.sourceType === "link_pipe" && code === 0 && signal === null && gotData) {
        this._log(
          `[relay:${this.sourceType}] Video finished (code=0 after ${uptimeSec}s, ` +
          `${kbRelayed} KB relayed) — stopping relay.`,
        );
        this._fatal(
          "[relay] Video source finished playing. Restart the stream to replay.",
        );
        return;
      }
