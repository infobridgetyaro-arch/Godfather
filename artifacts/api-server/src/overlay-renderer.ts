...
import { drawPlatformLogo } from "./social-logos";
  ...
  try {
    ...
    if (fontPath) {
      ...
      if (sansFontRegistered) {
        ...
        // YouTube/Sport) sets ctx.font with "…px sans-serif" directly, without
...
export interface OverlayState {
  ...
  disclaimerStyle: string;    // "Warning" | "Legal" | "Copyright" | "Viewer Advisory"
  disclaimerSoundUrl: string; // YouTube URL for browser-side audio (not rendered by this canvas)
  ...
  // ── Video output gradient (solid background fill for direct-stream platforms) ──
  videoOutputGradientActive: boolean;
  videoOutputGradient1: string;
  videoOutputGradient2: string;
  ...
  // ── Logo overlay ─────────────────────────────────────────────────────────
  logoActive: boolean;
  logoUrl: string;        // base64 data URL or http URL (single, legacy)
  ...
  // Multi-logo overlay – each has its own position, size, and animation
  logos?: Array<{ id: string; url: string; x: number; y: number; size: number; animation?: "None" | "Fade" | "Slide In" | "Zoom" | "Bounce" | "Spin"; animGap?: number }>;
  ...
  // ── Sub-headline (secondary line under motion modes) ─────────────────────
...
export function defaultOverlayState(): OverlayState {
  return {
    ...
    disclaimerText: "Some music in this stream may be protected by copyright. Viewer discretion is advised.",
    ...
    videoOutputGradientActive: false,
    videoOutputGradient1: "#003399",
    videoOutputGradient2: "#00cc44",
...
export class OverlayRenderer {
  ...
  // Logo overlay image cache (single, legacy)
  private logoImg: import("@napi-rs/canvas").Image | null = null;
  private logoImgSrc: string = "";

  // Multi-logo image cache — keyed by URL
  private logosImgCache = new Map<string, import("@napi-rs/canvas").Image | null>();
  ...
  // Dominant colour extracted from each cached avatar — used for gradient pills
  private avatarDominantColorCache = new Map<string, { r: number; g: number; b: number }>();
  ...
  // Per-logo animation start times (multi-logo path) — keyed by logo id
  private logosAnimStartT = new Map<string, number>();
  ...
  constructor(
    ...
    if (state.disclaimerActive) {
      this.alphas.disclaimer = 1;
    }
  ...
  updateState(patch: Partial<OverlayState>) {
    ...
    // Per-logo animation restart: when logoActive turns on, or when an individual
    // logo's URL or animation type changes, restart that logo's animation clock.
    if (patch.logoActive === true && !this.state.logoActive) {
      const t = this.elapsed();
      for (const logo of (patch.logos ?? this.state.logos ?? [])) {
        this.logosAnimStartT.set(logo.id, t);
      }
    } else if (patch.logos !== undefined) {
      const t = this.elapsed();
      const prev = this.state.logos ?? [];
      for (const logo of patch.logos) {
        const prevLogo = prev.find((l) => l.id === logo.id);
        if (
          !prevLogo ||
          (logo.url && logo.url !== prevLogo.url) ||
          (logo.animation !== undefined && logo.animation !== prevLogo.animation)
        ) {
          this.logosAnimStartT.set(logo.id, t);
        }
      }
    ...
    if (patch.chatBurnMessages) {
        ...
        if (url && !this.avatarCache.has(url)) {
          ...
          (loadImage as (src: string) => Promise<import("@napi-rs/canvas").Image>)(url)
            .then((img) => {
              ...
              this.avatarDominantColorCache.set(url, this.extractDominantColor(img));
            })
      ...
      if (this.avatarCache.size > 120) {
        ...
        for (const k of toDelete) {
          this.avatarCache.delete(k);
          this.avatarDominantColorCache.delete(k);
        }
      }
    }
  ...
  private renderFrame(): Buffer {
    ...
    if (this.renderMode === "bg") {
      if (this.state.breakActive) {
        ...
        // "live-bg": always draw video underlay — gradient always runs behind the video
    ...
    this.alphas.disclaimer  = this.stepAlpha(this.alphas.disclaimer,  this.state.disclaimerActive && !this.state.breakActive ? 1 : 0);
    ...
    this.withPanelAlpha(this.alphas.disclaimer * nonBreak, () => this.drawDisclaimer());
  ...
  /**
   * Sample the average pixel colour of a loaded avatar image by downscaling
   * to a 16×16 scratch canvas.  Returns an RGB triple.
   */
  private extractDominantColor(
    ...
    try {
      const sz  = 16;
      const tmp = createCanvas(sz, sz);
      const tc  = tmp.getContext("2d");
      tc.drawImage(img as never, 0, 0, sz, sz);
      const { data } = tc.getImageData(0, 0, sz, sz);
      let r = 0, g = 0, b = 0, n = 0;
      for (let i = 0; i < data.length; i += 4) {
        if (data[i + 3] < 80) continue; // skip near-transparent pixels
        r += data[i]; g += data[i + 1]; b += data[i + 2]; n++;
      }
      if (n === 0) return { r: 6, g: 182, b: 212 };
  ...
  /**
   * Return [leftStop, centerStop, rightStop] fixed gradient colours for the
   * Modern pill based on the active chat-burn style.
   *
   * Palette (left = darkest shadow edge, center = brand mid-tone, right = bright highlight):
   *   Bubble    → Royal Blue   #0B2A4A / #1565C0 / #2D9CFF
   *   Float     → Orange       #3D1500 / #C45000 / #FF8C2D
   *   Sidebar   → Green        #0A2E15 / #1A7A3C / #2DCC5E
   *   Highlight → Gold         #2E2000 / #B87800 / #FFD02D
   *   Ticker    → Indigo       #0D0B2E / #3730A3 / #818CF8
   */
  private getStyleGradient(style: string): [string, string, string] {
    switch (style) {
      case "Float":
        return ["#3D1500", "#C45000", "#FF8C2D"];   // Orange
      case "Sidebar":
        return ["#0A2E15", "#1A7A3C", "#2DCC5E"];   // Green
      case "Highlight":
        return ["#2E2000", "#B87800", "#FFD02D"];   // Gold / yellow
      case "Ticker":
        return ["#0D0B2E", "#3730A3", "#818CF8"];   // Indigo
      case "Bubble":
      default:
        return ["#0B2A4A", "#1565C0", "#2D9CFF"];   // Royal blue
    }
  }

  /**
   * Draw a pill-shaped card with a 3-stop left→right gradient, a top-shine,
   * and a drop shadow — matching the Social "Modern" card aesthetic.
   */
  private fillModernPill(
    x: number, y: number, w: number, h: number, r: number,
    leftStop: string, centerStop: string, rightStop: string,
  ): void {
    const { ctx } = this;
    // Drop shadow
    ctx.save();
    ...
    grad.addColorStop(0,   leftStop);
    grad.addColorStop(0.5, centerStop);
    grad.addColorStop(1,   rightStop);
    ctx.fillStyle = grad;
  ...
  private drawBubbleChat() {
    ...
    const rowH     = Math.round(H * 0.078);
    const gap      = Math.round(H * 0.010);
    const cardW    = Math.round(W * 0.44);
    const pillR    = Math.round(rowH * 0.50); // true pill
    ...
    const nameFs   = Math.round(rowH * 0.27);
    const msgFs    = Math.round(rowH * 0.25);

    ctx.textBaseline = "middle";
    const msgFont  = this.ef(`${msgFs}px sans-serif`);
    const nameFont = this.ef(`700 ${nameFs}px sans-serif`);
    ...
    msgs.forEach((msg, i) => {
      ...
      const [left, center, right] = this.getStyleGradient(this.state.chatBurnStyle);
      const cy = my + rowH / 2;

      // Modern gradient pill — style colour
      this.fillModernPill(bx, my, cardW, cardH, pillR, left, center, right);

      // Avatar circle
      this.drawAvatarCircle(avatarX, cy, avatarR, msg);

      // Text shadow — ensures legibility over any pill colour or video bleed
      ...
      // Message text — slightly translucent white
      ctx.font      = msgFont;
      ctx.fillStyle = "rgba(255,255,255,0.92)";
      if (multiLine) {
        const step   = Math.round(msgFs * 1.38);
  ...
  private drawBubbleChatMobile() {
    ...
    const rowH    = Math.round(H * 0.050);
    const gap     = Math.round(H * 0.007);
    const pillR   = Math.round(rowH * 0.50);
    const fontSize = Math.round(rowH * 0.34);
    ...
    preparedMsgs.forEach(({ msg, nameStr, nameW, lines }) => {
      ...
      // Text shadow for legibility over any pill colour or video bleed
      ctx.shadowColor   = "rgba(0,0,0,0.80)";
      ctx.shadowBlur    = 4;
      ctx.shadowOffsetX = 0;
      ctx.shadowOffsetY = 1;

      // Name inline (white bold)
  ...
  private drawFloatChat() {
    ...
    const pillR       = Math.round(cardH * 0.50); // true pill
    ...
    for (const msg of state.chatBurnMessages) {
      ...
      // Modern gradient pill — style colour
      const [left, center, right] = this.getStyleGradient(this.state.chatBurnStyle);
      this.fillModernPill(cardLeft, cardTop, cardW, cardH, pillR, left, center, right);
      ...
      // Text shadow — keeps white text legible over any pill colour
      ctx.shadowColor   = "rgba(0,0,0,0.80)";
      ctx.shadowBlur    = 4;
      ctx.shadowOffsetX = 0;
      ctx.shadowOffsetY = 1;
  ...
  private drawSidebarChat() {
    ...
    const headerH = Math.round(H * 0.040);
    ...
    const fontSize = Math.round(lineH * 0.34);
    const panelR   = Math.round(lineH * 0.50); // pill panel corners
    const rowGap   = Math.round(H * 0.006);
    ...
    this.fillRR(px2, py2, panelW, panelH, panelR);
    ctx.restore();
    // Panel top shine
    ctx.save();
    this.clipRR(px2, py2, panelW, panelH, panelR);
    const pShine = ctx.createLinearGradient(px2, py2, px2, py2 + panelH * 0.35);
    pShine.addColorStop(0, "rgba(255,255,255,0.08)");
    ...
    ctx.strokeStyle = "rgba(255,255,255,0.12)";
    ctx.lineWidth   = 1;
    this.strokeRR(px2, py2, panelW, panelH, panelR);

    // Header — gradient pill inside panel
    ctx.save();
    this.clipRR(px2, py2, panelW, headerH + panelR, panelR);
    const hGrad = ctx.createLinearGradient(px2, py2, px2 + panelW, py2);
    hGrad.addColorStop(0, "rgba(18,8,36,0.96)");
    hGrad.addColorStop(1, "rgba(60,20,90,0.90)");
    ctx.fillStyle = hGrad;
    ...
    // Each message row — small gradient pill
    const rowPillR = Math.round(lineH * 0.50);
    const rowPadX  = Math.round(lineH * 0.14);
    msgs.forEach((msg, i) => {
      const my = py2 + headerH + 9 + i * (lineH + rowGap);
      if (my + lineH > py2 + panelH - 4) return;

      const rx = px2 + rowPadX;
      const rw = panelW - rowPadX * 2;
      const [left, center, right] = this.getStyleGradient(this.state.chatBurnStyle);
      this.fillModernPill(rx, my, rw, lineH, rowPillR, left, center, right);

      const avatarCX = rx + rowPillR + avatarR;
      ...
      const nameStr  = msg.name + ":";
      let dName      = nameStr;
      const maxNameW = Math.round(rw * 0.38);
      ...
      const available = rw - (nameX - rx) - nameW - 8 - rowPillR;
      let txt = msg.text;
  ...
  private drawSidebarChatMobile() {
    ...
    const px2     = this.px(effPos.x, W);
    const py2     = this.px(effPos.y, H);
    const headerH = Math.round(H * 0.032);
    const lineH   = Math.round(H * 0.046);
    const avatarR = Math.round(lineH * 0.30);
    const MAX_SIDEBAR_MSGS_MOBILE = 7;
    const rowGap  = Math.round(H * 0.005);
    const panelH  = headerH + MAX_SIDEBAR_MSGS_MOBILE * (lineH + rowGap) + 12;
    const fontSize = Math.round(lineH * 0.34);
    const panelR   = Math.round(lineH * 0.50);
    const rowPillR = Math.round(lineH * 0.50);
    const rowPadX  = Math.round(lineH * 0.12);
    ...
    ctx.save();
    this.clipRR(px2, py2, panelW, panelH, panelR);
    const pShine = ctx.createLinearGradient(px2, py2, px2, py2 + panelH * 0.3);
    pShine.addColorStop(0, "rgba(255,255,255,0.07)");
    pShine.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = pShine;
    ctx.fillRect(px2, py2, panelW, panelH);
    ctx.restore();
    ...
    hg.addColorStop(0, "rgba(18,8,36,0.96)");
    hg.addColorStop(1, "rgba(60,20,90,0.90)");
    ctx.fillStyle = hg;
    ctx.fillRect(px2, py2, panelW, headerH);
    ctx.restore();
    ctx.fillStyle    = "#fff";
    ...
    msgs.forEach((msg, i) => {
      const my  = py2 + headerH + 6 + i * (lineH + rowGap);
      if (my + lineH > py2 + panelH - 4) return;
      ...
      const nameX = avatarCX + avatarR + 5;
      ctx.shadowColor   = "rgba(0,0,0,0.80)";
      ctx.shadowBlur    = 4;
      ctx.shadowOffsetX = 0;
      ctx.shadowOffsetY = 1;
      ...
      const nameStr = msg.name + ":";
      let dName     = nameStr;
      while (dName.length > 3 && ctx.measureText(dName).width > rw * 0.36)
        dName = dName.slice(0, -2) + "…";
      ctx.fillText(dName, nameX, avatarCY);
      const nameW = ctx.measureText(dName).width;
  ...
  private drawHighlightChat() {
    ...
    const bx     = this.px(effPos.x, W);
    const by     = this.px(effPos.y, H);
    const pillR  = Math.round(bh * 0.22); // pronounced rounded corners

    // Modern gradient pill — style colour
    const [left, center, right] = this.getStyleGradient(this.state.chatBurnStyle);
    this.fillModernPill(bx, by, bw, bh, pillR, left, center, right);
  ...
  /** Stationary — two-tier broadcast card: animated gradient BG, headline crossfades, sub-headline row. */
  private drawTickerStationary(t: number, yBase: number) {
    const { ctx, W, H, state } = this;
    const bh     = Math.max(44, Math.round(H * 0.066));   // headline row
    const sh     = Math.max(26, Math.round(H * 0.038));   // sub-headline row
    const totalH = bh + sh;
    const accent = this.themeAccentColor();
    const y      = yBase > 0 ? Math.min(H - totalH - 6, yBase) : H - totalH - 6;
    const ys     = y + bh;

    // ── Animated gradient headline bar ─────────────────────────────────────────
    const wave = Math.sin(t * 0.38) * 0.5 + 0.5;
    const bgG = ctx.createLinearGradient(0, y, W, y + bh);
    bgG.addColorStop(0, "rgba(4,4,14,0.98)");
    bgG.addColorStop(wave * 0.5 + 0.25, `rgba(${Math.round(wave * 16)},${Math.round(wave * 6)},${Math.round(wave * 28)},0.99)`);
    bgG.addColorStop(1, "rgba(4,4,14,0.98)");
    ctx.fillStyle = bgG; ctx.fillRect(0, y, W, bh);
    // Travelling shimmer
    const sx = ((t * 0.16) % 1.7) * W - 0.35 * W;
    const sg = ctx.createLinearGradient(sx - 130, 0, sx + 130, 0);
    sg.addColorStop(0, "transparent"); sg.addColorStop(0.5, `${accent}16`); sg.addColorStop(1, "transparent");
    ctx.fillStyle = sg; ctx.fillRect(0, y, W, bh);
    // Top gradient accent line
    const tg = ctx.createLinearGradient(0, 0, W, 0);
    tg.addColorStop(0, "transparent"); tg.addColorStop(0.12, accent); tg.addColorStop(0.88, accent); tg.addColorStop(1, "transparent");
    ctx.fillStyle = tg; ctx.fillRect(0, y, W, 2);

    // Badge with gradient fill
    const bw = Math.round(bh * 2.2);
    const badgeG = ctx.createLinearGradient(0, y, bw, y + bh);
    badgeG.addColorStop(0, accent); badgeG.addColorStop(1, `${accent}cc`);
    ctx.fillStyle = badgeG; ctx.fillRect(0, y, bw, bh);
    const label = (state.newsTitle || "LIVE").toUpperCase();
    ctx.fillStyle = "#fff"; ctx.font = `800 ${Math.round(bh * 0.33)}px sans-serif`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.shadowColor = "rgba(0,0,0,0.5)"; ctx.shadowBlur = 4;
    ctx.fillText(label, bw / 2, y + bh / 2); ctx.shadowBlur = 0; ctx.shadowColor = "transparent";
    // Pulsing dot in badge
    const dp = 0.5 + 0.5 * Math.sin(t * 4.8);
    ctx.save(); ctx.globalAlpha *= dp;
    ctx.fillStyle = "rgba(255,255,255,0.85)";
    ctx.beginPath(); ctx.arc(bw - 9, y + 9, 3, 0, Math.PI * 2); ctx.fill(); ctx.restore();

    // Headline text — crossfades every 6 s
    const HOLD_SECS = 6;
    ...
    const areaX = bw + 16; const areaW = W - areaX - 16;
    const fs = Math.round(bh * 0.36);
    ctx.save(); ctx.globalAlpha *= fadeAlpha;
    ctx.font = `600 ${fs}px sans-serif`; ctx.fillStyle = "#fff";
    ctx.textAlign = "left"; ctx.textBaseline = "middle";
    let txt = displayText;
    while (ctx.measureText(txt).width > areaW && txt.length > 4) txt = txt.slice(0, -4) + "…";
    ctx.fillText(txt, areaX, y + bh / 2); ctx.restore();

    // Gradient tier separator
    const sepG = ctx.createLinearGradient(0, 0, W, 0);
    sepG.addColorStop(0, "transparent"); sepG.addColorStop(0.08, `${accent}66`); sepG.addColorStop(0.92, `${accent}66`); sepG.addColorStop(1, "transparent");
    ctx.fillStyle = sepG; ctx.fillRect(0, ys, W, 1);

    // ── Sub-headline row ───────────────────────────────────────────────────────
    ctx.fillStyle = "rgba(5,5,16,0.97)"; ctx.fillRect(0, ys, W, sh);
    // Animated sweep gradient
    const swP = (t * 0.22) % 1;
    const swG = ctx.createLinearGradient(0, 0, W, 0);
    swG.addColorStop(0, "transparent");
    swG.addColorStop(Math.max(0, swP - 0.12), "transparent");
    swG.addColorStop(swP, `${accent}44`);
    swG.addColorStop(Math.min(1, swP + 0.12), "transparent");
    swG.addColorStop(1, "transparent");
    ctx.fillStyle = swG; ctx.fillRect(0, ys, W, sh);
    // Sub-headline text
    const subText = state.newsSubHeadline || "";
    if (subText) {
      const shFs = Math.round(sh * 0.44);
      ctx.font = `400 ${shFs}px sans-serif`;
      ctx.fillStyle = `${accent}cc`; ctx.textAlign = "left"; ctx.textBaseline = "middle";
      let sub = subText;
      while (ctx.measureText(sub).width > W - bw - 28 && sub.length > 4) sub = sub.slice(0, -4) + "…";
      ctx.fillText(sub, bw + 14, ys + sh / 2);
    }
    // Bottom accent line
    const btG = ctx.createLinearGradient(0, 0, W, 0);
    btG.addColorStop(0, accent); btG.addColorStop(0.6, `${accent}88`); btG.addColorStop(1, "transparent");
    ctx.fillStyle = btG; ctx.fillRect(0, ys + sh - 2, W, 2);
  }

  /** Flap — two-tier split-flap departure board: headline tiles + sub-headline row + clock. */
  private drawTickerFlap(t: number, yBase: number) {
    const { ctx, W, H, state } = this;
    const bh     = Math.max(42, Math.round(H * 0.068));
    const sh     = Math.max(26, Math.round(H * 0.038));
    const totalH = bh + sh;
    const accent = this.themeAccentColor();
    const y      = yBase > 0 ? Math.min(H - totalH - 6, yBase) : H - totalH - 6;
    const ys     = y + bh;
    ...
    // Label
    const bw = Math.round(bh * 2.2);
    ctx.fillStyle = "rgba(18,14,6,0.99)"; ctx.fillRect(0, y, bw, bh);
    ctx.fillStyle = accent; ctx.fillRect(bw, y, 2, bh);
    const label = (state.newsTitle || "NEWS").toUpperCase();
    ctx.fillStyle = accent; ctx.font = `700 ${Math.round(bh * 0.32)}px monospace`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText(label, bw / 2, y + bh / 2);

    // Flap character animation
    ...
    displayText.split("").forEach((targetChar, i) => {
      ...
      let display: string;
      if (!resolved) display = flapChars[Math.floor((Date.now() / 60 + i * 7) % flapChars.length)];
      else if (charProgress < 1) display = charProgress > 0.5 ? targetChar : flapChars[Math.floor((Date.now() / 40 + i * 3) % flapChars.length)];
      else display = targetChar;
      ...
      ctx.font = `700 ${fs}px monospace`;
      ctx.textAlign = "center"; ctx.textBaseline = "middle";
      ctx.translate(cx + charW / 2, y + bh / 2); ctx.scale(1, Math.max(0.05, flipScaleY));
      ctx.fillText(display, 0, 0); ctx.restore();
    });

    // ── Sub-headline row ───────────────────────────────────────────────────────
    ctx.fillStyle = "rgba(8,6,2,0.98)"; ctx.fillRect(0, ys, W, sh);
    ctx.fillStyle = `${accent}22`; ctx.fillRect(bw + 2, ys, W - bw - 2, sh);
    ctx.fillStyle = accent; ctx.fillRect(bw, ys, 2, sh);
    const shFs = Math.round(sh * 0.42);
    const subText = state.newsSubHeadline || "";
    if (subText) {
      ctx.font = `500 ${shFs}px monospace`;
      ctx.fillStyle = `${accent}dd`; ctx.textAlign = "left"; ctx.textBaseline = "middle";
      let sub = subText;
      while (ctx.measureText(sub).width > W - bw - 90 && sub.length > 4) sub = sub.slice(0, -4) + "…";
      ctx.fillText(sub, bw + 14, ys + sh / 2);
    }
    // Clock on right
    const now = new Date();
    const timeStr = now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
    ctx.font = `600 ${shFs}px monospace`;
    ctx.fillStyle = `${accent}99`; ctx.textAlign = "right"; ctx.textBaseline = "middle";
    ctx.fillText(timeStr, W - 12, ys + sh / 2);
    ctx.fillStyle = accent; ctx.fillRect(0, ys + sh - 2, W, 2);
  }

  /** Typewriter mode — two-tier terminal: headline types in, sub-headline follows with delay. */
  private drawTickerTypewriterMode(t: number, yBase: number) {
    ...
    // Terminal dark background with scanlines
    ctx.fillStyle = "rgba(2,6,2,0.98)"; ctx.fillRect(0, y, W, bh);
    for (let sl = y; sl < y + bh; sl += 4) { ctx.fillStyle = "rgba(0,255,0,0.018)"; ctx.fillRect(0, sl, W, 1); }
    ctx.fillStyle = accent; ctx.fillRect(0, y, W, 2);

    // Label badge
    const bw = Math.round(bh * 2.2);
    const badgeG = ctx.createLinearGradient(0, y, bw, y + bh);
    badgeG.addColorStop(0, accent); badgeG.addColorStop(1, `${accent}aa`);
    ctx.fillStyle = badgeG; ctx.fillRect(0, y, bw, bh);
    const label = (state.newsTitle || "LIVE").toUpperCase();
    ctx.fillStyle = "rgba(2,6,2,0.95)"; ctx.font = `700 ${Math.round(bh * 0.34)}px monospace`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText(label, bw / 2, y + bh / 2);
    ...
    const typeTime = Math.max(1, text.length / CHARS_PER_SEC);
    ...
    if (blink) {
      ...
      ctx.fillStyle = accent; ctx.fillRect(areaX + tw + 3, y + bh * 0.2, 2, bh * 0.6);
    }
    ctx.restore();

    // ── Sub-headline row ───────────────────────────────────────────────────────
    ctx.fillStyle = "rgba(2,5,2,0.98)"; ctx.fillRect(0, ys, W, sh);
    ctx.fillStyle = "rgba(74,222,128,0.04)"; ctx.fillRect(bw + 2, ys, W - bw - 2, sh);
    ctx.fillStyle = accent; ctx.fillRect(bw, ys, 2, sh);
    const subText = state.newsSubHeadline || "";
    if (subText) {
      const subDelay = typeTime * 0.5;
      const subVis = Math.max(0, Math.min(subText.length, Math.floor((cycleT - subDelay) * CHARS_PER_SEC)));
      ctx.save(); ctx.globalAlpha *= fadeAlpha;
      const shFs = Math.round(sh * 0.42);
      ctx.font = `400 ${shFs}px monospace`;
      ctx.fillStyle = "rgba(74,222,128,0.75)"; ctx.textAlign = "left"; ctx.textBaseline = "middle";
      let subTyped = subText.slice(0, subVis);
      while (subTyped.length > 1 && ctx.measureText(subTyped).width > W - bw - 28) subTyped = subTyped.slice(0, -1);
      ctx.fillText(subTyped, bw + 14, ys + sh / 2);
      // Sub cursor
      if (subVis < subText.length && Math.floor(Date.now() / 520) % 2 === 0) {
        const stw = ctx.measureText(subTyped).width;
        ctx.fillStyle = "rgba(74,222,128,0.7)"; ctx.fillRect(bw + 14 + stw + 2, ys + sh * 0.2, 1, sh * 0.6);
      }
      ctx.restore();
    }
    // Bottom status bar
    ctx.fillStyle = "rgba(22,101,52,0.55)"; ctx.fillRect(0, ys + sh - 13, W, 13);
    const now = new Date();
    const ts = now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });
    const statusFs = Math.round(sh * 0.30);
    ctx.font = `500 ${statusFs}px monospace`;
    ctx.fillStyle = "rgba(74,222,128,0.5)"; ctx.textAlign = "left"; ctx.textBaseline = "middle";
    ctx.fillText("● STREAM ACTIVE", bw + 14, ys + sh - 7);
    ctx.textAlign = "right"; ctx.fillText(ts, W - 12, ys + sh - 7);
  }

  /** Wave — two-tier: headline chars bob sinusoidally, sub-headline shows gradient text below. */
  private drawTickerWave(t: number, yBase: number) {
    ...
    ctx.fillStyle = "rgba(6,3,12,0.96)"; ctx.fillRect(0, y, W, bh);
    // Gradient top line (animated)
    const tg = ctx.createLinearGradient(0, y, W, y);
    tg.addColorStop(0, accent); tg.addColorStop(0.5, "#06b6d4"); tg.addColorStop(1, accent);
    ctx.fillStyle = tg; ctx.fillRect(0, y, W, 2);

    const bw = Math.round(bh * 2.2);
    ctx.fillStyle = "rgba(6,3,12,0.99)"; ctx.fillRect(0, y, bw, bh);
    ctx.fillStyle = accent; ctx.fillRect(bw, y, 2, bh);
    const label = (state.newsTitle || "LIVE").toUpperCase();
    ctx.fillStyle = accent; ctx.font = `700 ${Math.round(bh * 0.34)}px sans-serif`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ctx.fillText(label, bw / 2, y + bh / 2);
    ...
    if (cycleEl < scrollDur) {
      const scrollX = areaX + areaW - cycleEl * speed;
      const WAVE_AMP = bh * 0.18; const WAVE_FREQ = 0.008; const WAVE_SPEED = 2.5;
      ctx.save(); ctx.beginPath(); ctx.rect(areaX, y, areaW, bh); ctx.clip();
      let cx = scrollX; const baseY = y + bh / 2;
      text.split("").forEach((ch, i) => {
        ctx.font = this.ef(`600 ${fs}px sans-serif`);
        const cw = ctx.measureText(ch).width;
        if (cx + cw > areaX && cx < areaX + areaW) {
          const waveY = Math.sin(cx * WAVE_FREQ + t * WAVE_SPEED) * WAVE_AMP;
          const hue = (i * 5 + t * 30) % 360; const pct = (i % 8) / 8;
          ctx.fillStyle = pct < 0.5 ? "#fff" : `hsl(${hue},90%,78%)`;
          ctx.textAlign = "left"; ctx.textBaseline = "middle"; ctx.fillText(ch, cx, baseY + waveY);
        }
        cx += cw;
      });
      ctx.restore();
    ...
    // Gradient tier separator
    const sepG = ctx.createLinearGradient(0, 0, W, 0);
    sepG.addColorStop(0, "transparent"); sepG.addColorStop(0.08, `${accent}55`); sepG.addColorStop(0.92, `${accent}55`); sepG.addColorStop(1, "transparent");
    ctx.fillStyle = sepG; ctx.fillRect(0, ys, W, 1);

    // ── Sub-headline row ───────────────────────────────────────────────────────
    ctx.fillStyle = "rgba(5,2,10,0.97)"; ctx.fillRect(0, ys, W, sh);
    const subText = state.newsSubHeadline || "";
    if (subText) {
      const shFs = Math.round(sh * 0.42);
      ctx.font = `400 ${shFs}px sans-serif`;
      const textG = ctx.createLinearGradient(bw + 14, 0, Math.min(W - 12, bw + 400), 0);
      textG.addColorStop(0, accent); textG.addColorStop(0.5, "#06b6d4"); textG.addColorStop(1, accent);
      ctx.fillStyle = textG; ctx.textAlign = "left"; ctx.textBaseline = "middle";
      let sub = subText;
      while (ctx.measureText(sub).width > W - bw - 28 && sub.length > 4) sub = sub.slice(0, -4) + "…";
      ctx.fillText(sub, bw + 14, ys + sh / 2);
    }
    ctx.fillStyle = tg; ctx.fillRect(0, ys + sh - 2, W, 2);
  }

  /** Carousel — two-tier: headline slides vertically through messages, sub-headline with dot indicators. */
  private drawTickerCarousel(t: number, yBase: number) {
    ...
    const bgG = ctx.createLinearGradient(0, y, W, y + bh);
    bgG.addColorStop(0, "rgba(4,2,10,0.97)");
    bgG.addColorStop(wave * 0.4 + 0.3, `rgba(${Math.round(wave * 12)},${Math.round(wave * 4)},${Math.round(wave * 22)},0.98)`);
    bgG.addColorStop(1, "rgba(4,2,10,0.97)");
    ctx.fillStyle = bgG; ctx.fillRect(0, y, W, bh);
    // Shimmer
    const shimX = ((t * 0.20) % 1.6) * W - 0.3 * W;
    const shimG = ctx.createLinearGradient(shimX - 120, 0, shimX + 120, 0);
    shimG.addColorStop(0, "transparent"); shimG.addColorStop(0.5, `${accent}14`); shimG.addColorStop(1, "transparent");
    ctx.fillStyle = shimG; ctx.fillRect(0, y, W, bh);
    const tg = ctx.createLinearGradient(0, 0, W, 0);
    tg.addColorStop(0, "transparent"); tg.addColorStop(0.1, accent); tg.addColorStop(0.9, accent); tg.addColorStop(1, "transparent");
    ctx.fillStyle = tg; ctx.fillRect(0, y, W, 2);

    const bw = Math.round(bh * 2.2);
    const badgeG = ctx.createLinearGradient(0, y, bw, y + bh);
    badgeG.addColorStop(0, accent); badgeG.addColorStop(1, `${accent}bb`);
    ctx.fillStyle = badgeG; ctx.fillRect(0, y, bw, bh);
    const label = (state.newsTitle || "LIVE").toUpperCase();
    ctx.fillStyle = "#fff"; ctx.font = `800 ${Math.round(bh * 0.33)}px sans-serif`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle";
    ...
    ctx.fillText(label, bw / 2, y + bh / 2); ctx.shadowBlur = 0; ctx.shadowColor = "transparent";
    // Pulsing dot
    const dp = 0.5 + 0.5 * Math.sin(t * 4.5);
    ctx.save(); ctx.globalAlpha *= dp;
    ctx.fillStyle = "rgba(255,255,255,0.8)";
    ctx.beginPath(); ctx.arc(bw - 8, y + 9, 3, 0, Math.PI * 2); ctx.fill(); ctx.restore();
    ...
    const drawCarouselText = (txt: string, offY: number, alpha: number) => {
      let s = txt; ctx.font = `600 ${fs}px sans-serif`;
      while (ctx.measureText(s).width > areaW && s.length > 4) s = s.slice(0, -4) + "…";
      ctx.save(); ctx.beginPath(); ctx.rect(areaX, y, areaW, bh); ctx.clip();
      ctx.globalAlpha *= alpha; ctx.fillStyle = "#fff"; ctx.textAlign = "left"; ctx.textBaseline = "middle";
      ctx.fillText(s, areaX, y + bh / 2 + offY); ctx.restore();
    };

    if (isTransitioning) { drawCarouselText(curText, -slideY, 1 - easeP); drawCarouselText(nextText, bh - slideY, easeP); }
    else { drawCarouselText(curText, 0, 1); }
    ...
    // ── Sub-headline row ───────────────────────────────────────────────────────
    ctx.fillStyle = "rgba(4,2,10,0.97)"; ctx.fillRect(0, ys, W, sh);
    const subText = state.newsSubHeadline || "";
    if (subText) {
      const shFa = isTransitioning ? Math.max(0, 1 - easeP * 2) : 1;
      const shFs = Math.round(sh * 0.42);
      ctx.save(); ctx.globalAlpha *= shFa;
      ctx.font = `400 ${shFs}px sans-serif`;
      ctx.fillStyle = `${accent}cc`; ctx.textAlign = "left"; ctx.textBaseline = "middle";
      let sub = subText;
      while (ctx.measureText(sub).width > W - bw - 80 && sub.length > 4) sub = sub.slice(0, -4) + "…";
      ctx.fillText(sub, bw + 14, ys + sh / 2); ctx.restore();
    }
    // Dot indicators
    const dotCount = Math.min(5, lines.length);
    if (dotCount > 1) {
      for (let i = 0; i < dotCount; i++) {
        ctx.fillStyle = i === curIdx % dotCount ? accent : `${accent}40`;
        ctx.beginPath(); ctx.arc(W - 16 - (dotCount - 1 - i) * 10, ys + sh / 2, 3, 0, Math.PI * 2); ctx.fill();
      }
    }
    const btG = ctx.createLinearGradient(0, 0, W, 0);
    btG.addColorStop(0, accent); btG.addColorStop(0.6, `${accent}88`); btG.addColorStop(1, "transparent");
  ...
  // ── DISCLAIMER ─────────────────────────────────────────────────────────────
  //
  // Full-screen professional card overlay.  A deep blue-gradient backdrop fills
  // the entire canvas, and a centred rounded-rectangle card holds the badge,
  // divider, icon, and body text — giving the "curved edges" look.

  private drawDisclaimer() {
    const { ctx, W, H, state } = this;

    // ── Full-screen blue gradient backdrop ───────────────────────────────────
    const bg = ctx.createLinearGradient(0, 0, W, H);
    bg.addColorStop(0,    "rgba(5,15,50,0.93)");
    bg.addColorStop(0.45, "rgba(10,50,140,0.93)");
    bg.addColorStop(1,    "rgba(5,15,50,0.93)");
    ctx.fillStyle = bg;
    ctx.fillRect(0, 0, W, H);

    // ── Centred card ─────────────────────────────────────────────────────────
    const cardW = Math.round(W * (this.isVertical ? 0.88 : 0.72));
    const cardH = Math.round(H * (this.isVertical ? 0.50 : 0.56));
    ...
    ctx.save();
    ctx.shadowColor   = "rgba(0,0,0,0.60)";
    ctx.shadowBlur    = Math.round(W * 0.030);
    ctx.shadowOffsetY = Math.round(H * 0.014);
    const cardFill = ctx.createLinearGradient(cardX, cardY, cardX + cardW, cardY + cardH);
    cardFill.addColorStop(0, "rgba(18,50,130,0.98)");
    cardFill.addColorStop(1, "rgba(8,28,90,0.98)");
    ctx.fillStyle = cardFill;
    this.roundRect(cardX, cardY, cardW, cardH, r);
    ctx.fill();
    ctx.restore();

    // Card border — soft blue glow
    ctx.save();
    ...
    ctx.save();
    const hl = ctx.createLinearGradient(cardX, cardY, cardX + cardW, cardY);
    hl.addColorStop(0,   "rgba(80,160,255,0.0)");
    hl.addColorStop(0.3, "rgba(80,160,255,0.45)");
    hl.addColorStop(0.7, "rgba(80,160,255,0.45)");
    hl.addColorStop(1,   "rgba(80,160,255,0.0)");
    ctx.strokeStyle = hl;
    ctx.lineWidth   = 1;
    ctx.beginPath();
    ctx.moveTo(cardX + r, cardY + 1);
    ctx.lineTo(cardX + cardW - r, cardY + 1);
    ctx.stroke();
    ctx.restore();

    // ── Style config ─────────────────────────────────────────────────────────
    const cfg = this._disclaimerStyleConfig(state.disclaimerStyle);

    // ── Badge pill ───────────────────────────────────────────────────────────
    // ef() injects "'Noto Color Emoji', 'Noto Sans'" so emoji glyphs render.
    const badgeFontSz = Math.round(Math.min(W, H) * (this.isVertical ? 0.028 : 0.020));
    ctx.font = this.ef(`700 ${badgeFontSz}px sans-serif`);
    const pillPadX  = Math.round(badgeFontSz * 1.3);
    const pillPadY  = Math.round(badgeFontSz * 0.55);
    const pillW     = ctx.measureText(cfg.label).width + pillPadX * 2;
    const pillH     = badgeFontSz + pillPadY * 2;
    const pillX     = cardX + (cardW - pillW) / 2;
    const pillY     = cardY + Math.round(cardH * 0.08);
    ...
    ctx.textAlign    = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(cfg.label, pillX + pillW / 2, pillY + pillH / 2);

    // ── Divider ───────────────────────────────────────────────────────────────
    const divY  = pillY + pillH + Math.round(cardH * 0.05);
    const divGr = ctx.createLinearGradient(cardX, divY, cardX + cardW, divY);
    divGr.addColorStop(0,   "rgba(80,140,255,0.0)");
    divGr.addColorStop(0.25,"rgba(80,140,255,0.5)");
    divGr.addColorStop(0.75,"rgba(80,140,255,0.5)");
    divGr.addColorStop(1,   "rgba(80,140,255,0.0)");
    ctx.fillStyle = divGr;
    ctx.fillRect(cardX + Math.round(cardW * 0.05), divY, Math.round(cardW * 0.90), 1);

    // ── Icon — emoji rendered via Noto Color Emoji ────────────────────────────
    const iconSz = Math.round(Math.min(W, H) * (this.isVertical ? 0.052 : 0.040));
    const iconCY = divY + Math.round(cardH * 0.10) + iconSz / 2;
    ctx.font         = this.ef(`${iconSz}px sans-serif`);
    ctx.fillStyle    = cfg.iconColor;
    ctx.textAlign    = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(cfg.icon, cardX + cardW / 2, iconCY);

    // ── Body text — word-wrapped, centered ───────────────────────────────────
    const bodyFontSz = Math.round(Math.min(W, H) * (this.isVertical ? 0.026 : 0.019));
    ctx.font         = this.ef(`${bodyFontSz}px sans-serif`);
    ctx.fillStyle    = "rgba(210,228,255,0.95)";
    ctx.textAlign    = "center";
    ctx.textBaseline = "top";
    const bodyMaxW   = Math.round(cardW * 0.82);
    const bodyX      = cardX + cardW / 2;
    const bodyY      = iconCY + iconSz * 0.6 + Math.round(cardH * 0.04);
    const lineH      = Math.round(bodyFontSz * 1.60);
    const lines      = this.wrapTextToLines(ctx, state.disclaimerText, bodyMaxW, 4);
    for (let i = 0; i < lines.length; i++) {
      ctx.fillText(lines[i]!, bodyX, bodyY + i * lineH);
    }
  }

  private _disclaimerStyleConfig(style: string): {
    label: string; icon: string; badgeBg: string; iconColor: string;
  } {
    // U+FE0F (variation selector-16) after each symbol forces emoji presentation
    // so @napi-rs/canvas picks Noto Color Emoji instead of the text-mode glyph.
    switch (style) {
      case "Legal":
        return { label: "LEGAL DISCLAIMER", icon: "\u2696\uFE0F", badgeBg: "rgba(37,99,235,0.90)",  iconColor: "#93c5fd" };
      case "Copyright":
        return { label: "COPYRIGHT NOTICE", icon: "\u00A9\uFE0F", badgeBg: "rgba(124,58,237,0.90)", iconColor: "#c4b5fd" };
      case "Viewer Advisory":
        return { label: "VIEWER ADVISORY",  icon: "\uD83D\uDC41\uFE0F", badgeBg: "rgba(220,38,38,0.90)", iconColor: "#fca5a5" };
      default: // "Warning"
        return { label: "WARNING",          icon: "\u26A0\uFE0F", badgeBg: "rgba(217,119,6,0.90)",  iconColor: "#fcd34d" };
    }
  }
  ...
  /** Load / refresh images for all items in the logos array. Fire-and-forget. */
  private async ensureLogosImgs(): Promise<void> {
    const logos = this.state.logos;
    if (!logos || logos.length === 0) return;
    const { loadImage } = await import("@napi-rs/canvas");
    for (const logo of logos) {
      if (!logo.url) continue;
      if (this.logosImgCache.has(logo.url)) continue; // already loaded or loading
      // Mark as pending so we don't start multiple concurrent loads for the same URL
      this.logosImgCache.set(logo.url, null);
      try {
        const img = await loadImage(logo.url);
        this.logosImgCache.set(logo.url, img);
      } catch {
        // Keep null — will just skip this logo
      }
    }
  }
  ...
  /** Compute animation modifiers for a logo draw call.
   *  @param anim  animation type string
   *  @param gap   pause between plays in seconds
   *  @param animStartT  elapsed-time value when this logo's animation began
   */
  private _logoAnimMods(targetW: number, anim: string, gap: number, animStartT: number): { animAlpha: number; animScale: number; animRotate: number; animOffsetX: number; animOffsetY: number } {
    const t = this.elapsed();
    const dt = Math.max(0, t - animStartT);
    const cycle = ANIM_DUR + gap;
    const phase = cycle > 0 ? dt % cycle : 0;
    const progress = Math.min(1, phase / ANIM_DUR);
    ...
    if (anim === "Fade") {
      ...
      const b = progress < 1 ? 1 - Math.pow(2, -8 * progress) * Math.cos(progress * 11) : 1;
      animScale = Math.max(0, b);
    } else if (anim === "Spin") {
      animRotate = progress * Math.PI * 2;
    }
    return { animAlpha, animScale, animRotate, animOffsetX, animOffsetY };
  }

  /** Draw a single logo image at the given position/size (shared by legacy and multi-logo paths). */
  private _drawLogoImage(
    img: import("@napi-rs/canvas").Image,
    xPct: number, yPct: number, sizePx: number, opacity: number,
    anim: string, animGap: number, animStartT: number,
  ): void {
    const { ctx, W, H, state } = this;
    const targetH = sizePx * (H / 1080);
    const targetW = (img.width / img.height) * targetH;
    const x = xPct / 100 * W;
    const y = yPct / 100 * H;
    const cx = x, cy = y + targetH / 2;

    const { animAlpha, animScale, animRotate, animOffsetX, animOffsetY } = this._logoAnimMods(targetW, anim, animGap, animStartT);

    ctx.save();
    ctx.globalAlpha = (this._panelAlpha ?? 1) * (opacity / 100) * animAlpha;
  ...
  private drawLogo(): void {
    const { state } = this;
    if (!state.logoActive) return;

    // ── Multi-logo path ───────────────────────────────────────────────────────
    const logos = state.logos;
    if (logos && logos.length > 0) {
      void this.ensureLogosImgs();
      for (const logo of logos) {
        if (!logo.url) continue;
        const img = this.logosImgCache.get(logo.url);
        if (!img || !img.width || !img.height) continue;
        const animStartT = this.logosAnimStartT.get(logo.id) ?? -100;
        this._drawLogoImage(
          img, logo.x ?? 85, logo.y ?? 3, logo.size ?? 120, state.logoOpacity ?? 100,
          logo.animation ?? "Fade", logo.animGap ?? 2.5, animStartT,
        );
      }
      return;
    }

    // ── Legacy single-logo path (backward compat) ─────────────────────────────
    if (!state.logoUrl) return;
    void this.ensureLogoImg();
    const img = this.logoImg;
    if (!img || !img.width || !img.height) return;
    this._drawLogoImage(
      img, state.logoX ?? 85, state.logoY ?? 3, state.logoSize ?? 120, state.logoOpacity ?? 100,
      state.logoAnimation ?? "Fade", state.logoAnimGap ?? 2.5, this.logoAnimStartT,
    );
  }
  ...
  private static readonly SOCIAL_PLATFORMS: Record<string, { bg: string; accent: string; icon: string }> = {
    // Icons use ASCII / basic Latin only — Unicode symbols (♪ ◉ ▶ etc.) render
    // as "?" on this server because the system sans-serif font does not cover the
    // Geometric Shapes / Miscellaneous Symbols Unicode blocks.  Facebook's "f"
    // always worked because it is plain ASCII.
    TikTok:    { bg: "#010101",   accent: "#ee1d52", icon: "Tk" },
    Instagram: { bg: "#833ab4",   accent: "#f77737", icon: "Ig" },
    Facebook:  { bg: "#1877F2",   accent: "#0d5bb5", icon: "f"  },
    YouTube:   { bg: "#FF0000",   accent: "#990000", icon: "Yt" },
    X:         { bg: "#14171a",   accent: "#1d9bf0", icon: "X"  },
    Twitter:   { bg: "#14171a",   accent: "#1d9bf0", icon: "Tw" },
    Twitch:    { bg: "#9146FF",   accent: "#6441a5", icon: "Tv" },
    Snapchat:  { bg: "#FFFC00",   accent: "#FFEC00", icon: "Sn" },
    LinkedIn:  { bg: "#0A66C2",   accent: "#004182", icon: "in" },
    Discord:   { bg: "#5865F2",   accent: "#4752c4", icon: "Dc" },
  };
  ...
  private drawSocialOverlay(t: number): void {
    ...
    // ── Unified entry/exit progress: 0 = hidden, 1 = fully visible ──────────
    let progress = 1.0;
    if (phaseT < IN_DUR) {
      const p = phaseT / IN_DUR;
      progress = this.easeElastic(p);
    } else if (phaseT >= SHOW_DUR - OUT_DUR) {
      const p = (phaseT - (SHOW_DUR - OUT_DUR)) / OUT_DUR;
      progress = 1 - this.easeInOut(p);
    }
    ...
    // ── Per-animation-type transform values ──────────────────────────────────
    // socialAnimation was previously stored and returned by the API but never
    // read by the renderer — every style fell through to the Slide path.
    const anim      = (state.socialAnimation ?? "Slide") as string;
    const prog01    = Math.max(0, Math.min(1.05, progress));  // clamped 0–1.05
    let   px        = baseX;           // card left edge X (Slide shifts this)
    let   animScX   = 1.0;             // extra canvas scaleX
    let   animScY   = 1.0;             // extra canvas scaleY
    let   animRot   = 0;               // extra canvas rotation (radians)
    let   animAlpha = 1.0;             // extra alpha multiplier

    switch (anim) {
      default:
      case "Slide":
        // Horizontal slide from the left — original behaviour.
        px = baseX + (1 - prog01) * -(totalW + baseX + 24);
        break;
      case "Pop":
        // Scale up from zero at the card's rest position.
        animScX = animScY = prog01;
        animAlpha = Math.min(1, progress * 2);
        break;
      case "Spin":
        // Full 360° spin while fading/scaling in.
        animRot   = (1 - prog01) * -Math.PI * 2;
        animScX   = animScY = prog01;
        animAlpha = Math.min(1, progress * 2);
        break;
      case "Flip":
        // Horizontal flip: scaleX swings from 0 → 1 (like flipping a card face-up).
        animScX   = prog01;
        animAlpha = Math.min(1, progress * 2);
        break;
      case "Pulse":
        // Fade in with an elastic scale bounce.
        animAlpha = Math.min(1, progress);
        animScX   = animScY = 0.7 + 0.3 * this.easeElastic(Math.min(1, progress));
        break;
    }
    ...
    ctx.save();
    ctx.globalAlpha *= this._panelAlpha * animAlpha;

    // Apply the per-animation canvas transform (Slide only moves px; the others
    // use a scale/rotate around the card centre so the card stays anchored).
    if (anim !== "Slide") {
      const cx = px + totalW / 2;
      const cy = py + cardH  / 2;
      ctx.translate(cx, cy);
      ctx.rotate(animRot);
      ctx.scale(animScX, animScY);
      ctx.translate(-cx, -cy);
    }
    ...
    if (cardStyle === "Glass") {
      ...
      // Icon
      drawPlatformLogo(ctx, cur.platform, px + slabW / 2, py + cardH / 2, Math.round(cardH * 0.55));
      // Platform label
      ...
      // Icon
      drawPlatformLogo(ctx, cur.platform, px + slabW / 2, py + cardH / 2, Math.round(cardH * 0.55), isSnap ? "#000" : "#fff");
      ...
      ctx.shadowBlur = 0;
      // Icon (left zone)
      drawPlatformLogo(ctx, cur.platform, px + slabW / 2, py + cardH / 2, Math.round(cardH * 0.55), isSnap ? "#000000" : "#ffffff");
      // Platform label
      ...
      ctx.shadowBlur  = 12;
      ctx.shadowColor = platform.accent;
      drawPlatformLogo(ctx, cur.platform, px + slabW / 2, py + cardH / 2, Math.round(cardH * 0.55), platform.accent);
      ctx.shadowBlur = 0; ctx.shadowColor = "transparent";
      // Divider line
      ctx.fillStyle = platform.accent;
