...
export function normaliseFacebookUrl(input: string): string | null {
  ...
  // fb.watch/XXXXXXXX shortlinks — yt-dlp follows the redirect to the video page.
  // These are the most common sharing format and do not have a numeric ID in the URL.
  if (/^https?:\/\/fb\.watch\//i.test(withScheme)) return withScheme;

  // New-style Facebook share URLs (e.g. facebook.com/share/v/XXXXXXX,
  // facebook.com/share/p/XXXXXXX, facebook.com/share/r/XXXXXXX) —
  // yt-dlp resolves these correctly.
  if (/\/share\/[vpr]\/[a-zA-Z0-9_-]+/i.test(withScheme)) return withScheme;

  // facebook.com/reel/ URLs (short-form video)
  if (/\/reel\/\d+/i.test(withScheme)) return withScheme;
  ...
  const hasVideoId = /[?&/]v=\d+|\/videos\/\d+|\/video\/\d+|video\.php\?.*v=\d+|\/live\/videos\/\d+/i.test(withScheme);
