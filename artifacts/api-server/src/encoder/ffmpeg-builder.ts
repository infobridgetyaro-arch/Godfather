...
export function buildSourceInputArgs(
  ...
  const isPipe    =
    resolvedSourceType === "tiktok_pipe" ||
    resolvedSourceType === "youtube_pipe" ||
    resolvedSourceType === "facebook_pipe" ||
    resolvedSourceType === "link_pipe";
...
export function buildFFmpegArgs(opts: BuildFFmpegArgsOptions): string[] {
  ...
  // thread_queue_size: default is 4 for rawvideo — far too small; raises to 512
  // to prevent "Thread message queue blocking" warnings that stall the encoder.
  args.push(
  ...
  // ── Video encoder ─────────────────────────────────────────────────────────
  // Default: "ultrafast" — removes lookahead buffers and minimises CPU, keeping
  // speed consistently ≥ 1.0× on shared VPS hardware. "veryfast"/"superfast"
  // pushed CPU above 100 % and caused sustained 0.73× speed and buffering.
