...
export class BottleneckAnalyzer {
  ...
  private scoreCpuEncoder(m: PipelineMetrics): BottleneckScore {
    ...
    // Low CPU with slow speed is a strong sign of pipe/queue backpressure, not
    // CPU overload. The encoder is blocked waiting for input frames, not burning
    // cycles. Penalise to avoid triggering unnecessary bitrate cuts.
    if (speed < 0.85 && cpu < 50) {
      score = Math.max(0, score - 20);
      evidence.push(`Low CPU (${cpu.toFixed(0)}%) with slow speed — likely pipe stall, not encoder overload`);
    }
  ...
  // ── Pipe Stall / Queue Backpressure ─────────────────────────────────────────
  // Distinguishes rawvideo/audio pipe blocking from true CPU encoder overload.
  // Pattern: encoder speed well below 1× but CPU is moderate (<55%) and FPS is
  // normal (>10) — the encoder is waiting on a blocked pipe read, not burning
  // CPU cycles. Common cause: thread_queue_size too small on a rawvideo input,
  // causing FFmpeg's demuxer thread to stall and back-pressure the entire graph.
  private scorePipeStall(m: PipelineMetrics): BottleneckScore {
    const subsystem: BottleneckSubsystem = "pipe_stall";
    const evidence: string[] = [];
    let score = 0;
    ...
    // Core signal: slow encoder + low CPU + frame production is normal.
    // If the CPU were the bottleneck, CPU would be high. If it were input
    // starvation, FPS would be very low. Pipe stall is the remaining cause.
    if (speed < 0.85 && cpu < 55 && m.fps30s > 10) {
      score += 40;
      evidence.push(`Speed ${speed.toFixed(2)}x low with CPU ${cpu.toFixed(0)}% and FPS ${m.fps30s.toFixed(0)} normal — pipe/queue backpressure likely`);
    }
    if (speed < 0.75 && cpu < 45) {
      score += 25;
      evidence.push(`Speed critically slow (${speed.toFixed(2)}x) at low CPU (${cpu.toFixed(0)}%) — input pipe blocking encoder thread`);
    }
    // Speed improving slowly after a restart is a recovery signal, not pipe stall.
    if (m.speedTrend === "improving") {
      score = Math.max(0, score - 15);
      evidence.push("Speed trend improving — transient startup lag, not sustained stall");
    }

    return {
      label: "Pipe/Queue Backpressure",
      subsystem,
      confidence: Math.min(100, Math.max(0, score)),
      evidence,
    };
