 ...
 *  Stage 1 — bitrate −15%                         (90s window — requires sustained overload)
 *  Stage 2 — bitrate −25% + fps step              (120s window)
 *  Stage 3 — resolution step-down + bitrate       (180s window)
 *  Stage 4 — fps step 2 at reduced resolution     (180s window)
 *  Stage 5 — resolution step 2 + min fps          (240s window)
 *
 * Philosophy: stream continuity > quality reduction. A 2–5 s dead-air restart
 * is far worse for viewers than running slightly above ideal CPU. Monitoring
 * windows are intentionally long (90–240 s) and speed thresholds intentionally
 * low (< 0.80x) so the AQM only fires during a genuinely sustained collapse,
 * not on the routine CPU spikes that happen when overlays or features join.
 */
...
const STAGE_LADDER: StageSpec[] = [
  {
    ...
    // 45 s window: act faster when speed drops below 1.0× to prevent buffering.
    // Brief CPU spikes (overlay joins, scene transitions) still cancel naturally
    // via the speed-improving hysteresis check above.
    monitorWindowMs: 45_000,
    cooldownMs: 90_000,
  },
  {
    stage: 2,
    ...
    monitorWindowMs: 120_000,  // 2 min — only act if Stage 1 didn't help
    cooldownMs: 120_000,
  },
  {
    stage: 3,
...
// ── Speed thresholds ──────────────────────────────────────────────────────────
//
// Target ≥ 1.0× to prevent viewer-side buffering.  Monitoring starts at 0.95×
// so AQM has time to degrade before the stream actually stutters.  Cancels
// monitoring when speed recovers to 1.0× (SPEED_NOMINAL).
//
const SPEED_FLOOR          = 0.95;   // below this → start monitoring (buffer risk)
const SPEED_SEVERE         = 0.68;   // shorten monitor window to 20s (truly severe)
const SPEED_CRITICAL       = 0.55;   // shorten monitor window to 15s (encoder collapsing)
const SPEED_RECOVERY       = 1.02;   // sustain above this to attempt quality restore
const SPEED_NOMINAL        = 1.00;   // above this → cancel active monitoring window

const RECOVERY_HOLD_MS     = 2 * 60_000;    // 2 min hold before restoring stage
...
export class AdaptiveQualityManager {
  ...
  feed(sample: AQMSample): AQMDecision {
    ...
    // ── Force mode: allow bitrate-only stages to protect speed floor ─────────
    // Resolution and FPS are locked in force mode, but if speed drops below
    // SPEED_FLOOR the encoder will buffer regardless.  Stages with no fps or
    // resolution reduction (fpsReduction === 0 && resolutionSteps === 0) are
    // safe to apply — they only reduce bitrate, leaving the locked quality intact.
    if (this.mode !== "auto") {
      const nextSpec = STAGE_LADDER[this.currentStage];
      const canActInForceMode =
        nextSpec !== undefined &&
        nextSpec.fpsReduction === 0 &&
        nextSpec.resolutionSteps === 0;
      if (!canActInForceMode) {
        return this.noOp(logLines, dashboardLog);
      }
      // Fall through to the normal monitoring/degrade path below (bitrate only).
    }
