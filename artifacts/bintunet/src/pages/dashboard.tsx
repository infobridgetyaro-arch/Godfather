...
const AIPanel       = lazy(() => import("@/components/control-room/ai-panel").then(m => ({ default: m.AIPanel })));
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const unsubCountdown = subscribe("reconnect_countdown", (msg) => {
      if (msg.streamId && msg.data) {
        // Server sends remainingMs (milliseconds); store raw ms for ms-precision display
        const remaining = (msg.data.remainingMs ?? (msg.data.remainingSec != null ? msg.data.remainingSec * 1000 : 0)) as number;
  ...
  const clearAllStreams = async () => {
    if (streams.length === 0) return;
    if (!window.confirm(`Remove all ${streams.length} stream${streams.length !== 1 ? "s" : ""}? This clears all configs and stops any active streams.`)) return;
    setClearingAll(true);
    const ids = streams.map((s) => s.id);
    await Promise.allSettled(ids.map((id) => apiRequest("DELETE", `/api/streams/${id}`)));
    setStreams([]);
    setStreamLogs({});
    setStreamStats({});
    setStreamChat({});
    setStreamProcStats({});
    ids.forEach((id) => delete seenChatIds[id]);
    clearDrafts();
    setClearingAll(false);
    toast({ title: "Dashboard cleared", description: "All stream configs removed." });
  };
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-5xl mx-auto px-4 py-2 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Button
              ...
              className={`w-7 h-7 ${clearingAll ? "opacity-50" : "hover:text-destructive hover:bg-destructive/10"}`}
              data-testid="button-clear-all"
              title="Clear all streams"
              disabled={clearingAll || streams.length === 0}
            >
      ...
      {showAI && (
        <div
          ...
          style={{
            ...
            boxShadow: "0 12px 48px rgba(0,0,0,0.7), 0 0 0 1px rgba(124,58,237,0.08)",
          ...
          <div style={{ padding: "14px 16px 12px" }}>
            <Suspense fallback={
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 120 }}>
                <Loader2 style={{ width: 20, height: 20, animation: "spin 1s linear infinite", opacity: 0.4 }} />
              </div>
            }>
              <AIPanel activeStreamCount={activeCount} />
            </Suspense>
          </div>
        </div>
      )}
      ...
      <button
        ...
        style={{
          ...
          border: `1.5px solid ${showAI ? "rgba(124,58,237,0.6)" : "rgba(124,58,237,0.45)"}`,
          boxShadow: showAI
        ...
        aria-label={showAI ? "Close AI Assistant" : "AI Assistant"}
      >
        {showAI ? (
