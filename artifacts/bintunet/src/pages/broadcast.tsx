...
interface NewsOverlayLive {
  ...
  headline: { text: string; subHeadline: string; animation: string; durationMs: number; autoRotate: boolean; headlines: string[]; currentIndex: number };
...
function TickerDisplay({ text, speed, color, fontSize, fontWeight, separator, mode, subHeadline, accentColor }: {
  text: string; speed?: number; color?: string; fontSize?: number; fontWeight?: number; separator?: string; mode?: string; subHeadline?: string; accentColor?: string;
}) {
  if (!mode || mode === "Scroll") {
    return <TickerScroll text={text} speed={speed} color={color} fontSize={fontSize} fontWeight={fontWeight} separator={separator} />;
  }

  const accent = accentColor || "#cc0001";
  ...
  const subRow: React.CSSProperties = {
    height: 22, flexShrink: 0, display: "flex", alignItems: "center", padding: "0 12px",
    background: "rgba(4,4,14,0.97)", borderTop: `1px solid ${accent}33`,
    fontSize: (fontSize || 14) * 0.72, color: `${accent}cc`, fontWeight: 400,
    ...
    return (
      <div style={wrap}>
        <div style={headlineRow}>
          <span key={text} style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontSize, fontWeight, color, animation: "td-fade 8s ease infinite" }}>{text}</span>
        </div>
        {subHeadline && <div style={subRow}>{subHeadline}</div>}
  ...
  if (mode === "Flap") {
    return (
      <div style={wrap}>
        <div style={{ ...headlineRow, perspective: 600 }}>
          <span key={text} style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontSize, fontWeight, color, fontFamily: "monospace", display: "inline-block", transformOrigin: "center", animation: "td-flap 0.55s cubic-bezier(0.4,0,0.2,1) both" }}>{text}</span>
        </div>
        {subHeadline && <div style={{ ...subRow, fontFamily: "monospace" }}>{subHeadline}</div>}
        <style>{`@keyframes td-flap { 0%{transform:rotateX(90deg);opacity:0} 50%{transform:rotateX(-8deg);opacity:1} 100%{transform:rotateX(0deg);opacity:1} }`}</style>
      </div>
    );
  }
  ...
  if (mode === "Typewriter") {
    return (
      <div style={wrap}>
        <div style={headlineRow}>
          <span key={text} style={{ whiteSpace: "nowrap", fontSize, fontWeight, color: "#4ade80", fontFamily: "monospace", textShadow: "0 0 6px #22c55e66", animation: "td-type 3.5s steps(45,end) both" }}>{text}</span>
        </div>
        {subHeadline && <div style={{ ...subRow, color: "rgba(74,222,128,0.6)", fontFamily: "monospace", background: "rgba(2,6,2,0.97)" }}>{subHeadline}</div>}
  ...
  if (mode === "Wave") {
    return (
      <div style={wrap}>
        <div style={headlineRow}>
          <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontSize, fontWeight, color, display: "inline-block", animation: "td-wave 2s ease-in-out infinite" }}>{text}</span>
        </div>
        {subHeadline && <div style={{ ...subRow, color: `${accent}cc`, background: "rgba(5,2,10,0.97)" }}>{subHeadline}</div>}
  ...
  if (mode === "Carousel") {
    return (
      <div style={wrap}>
        <div style={{ ...headlineRow, overflow: "hidden" }}>
          <span key={text} style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontSize, fontWeight, color, display: "inline-block", animation: "td-carousel 7s cubic-bezier(0.4,0,0.2,1) both" }}>{text}</span>
        </div>
        {subHeadline && <div style={subRow}>{subHeadline}</div>}
        <style>{`@keyframes td-carousel { 0%{transform:translateY(110%);opacity:0} 12%{transform:translateY(0);opacity:1} 88%{transform:translateY(0);opacity:1} 100%{transform:translateY(-110%);opacity:0} }`}</style>
      </div>
    );
  }
...
/* ─── News overlay renderer (API-driven) ─── */
...
function NewsOverlayRenderer({ overlay, isMobile, newsScale, tickerMode }: { overlay: NewsOverlayLive; isMobile: boolean; newsScale?: number; tickerMode?: string }) {
  ...
  const h = Math.round((isMobile ? 38 : 46) * scale);
  ...
  const subHeadline = overlay.headline?.subHeadline || "";
  ...
  const themeStyles: Record<string, React.ReactNode> = {
    "Al Jazeera": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color={textColor} fontSize={fontSize} fontWeight={font?.weight || 600} mode={tickerMode} />
      </div>
    ),
    "CNN": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "#0d0d0d" }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color="#f0f0f0" fontSize={fontSize} fontWeight={font?.weight || 700} mode={tickerMode} />
      </div>
    ),
    "BBC": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "#1a1a2e" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", padding: `0 ${isMobile ? 8 : 12}px`, flex: 1, overflow: "hidden", gap: 2 }}>
          ...
          <TickerDisplay text={displayText} speed={speed} color={textColor} fontSize={fontSize} fontWeight={font?.weight || 700} separator="  ·  " mode={tickerMode} />
        </div>
      </div>
    ),
    "Bloomberg": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "#0c0c0c", borderTop: `2px solid ${c}` }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color="rgba(255,255,255,0.88)" fontSize={fontSize} fontWeight={font?.weight || 500} separator="  |  " mode={tickerMode} />
      </div>
    ),
    "Fox": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "#000033", borderTop: `3px solid ${c}` }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color="#fff" fontSize={fontSize} fontWeight={font?.weight || 800} mode={tickerMode} />
      </div>
    ),
    "Sky News": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h }}>
    ...
    "CNBC": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "#000022", borderTop: `2px solid ${c}` }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color={textColor} fontSize={fontSize} fontWeight={font?.weight || 700} mode={tickerMode} />
      </div>
    ),
    "Dark": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "rgba(10,10,20,0.97)", border: "1px solid rgba(99,102,241,0.4)" }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color="rgba(255,255,255,0.9)" fontSize={fontSize} fontWeight={font?.weight || 600} mode={tickerMode} />
      </div>
    ),
    "Glass": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "rgba(255,255,255,0.08)", backdropFilter: "blur(16px)", border: "1px solid rgba(255,255,255,0.14)", borderRadius: isMobile ? 6 : 8, overflow: "hidden" }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color="rgba(255,255,255,0.92)" fontSize={fontSize} fontWeight={font?.weight || 600} mode={tickerMode} />
      </div>
    ),
    "Modern": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "rgba(0,4,16,0.97)", borderTop: `2px solid ${c}`, boxShadow: `0 -4px 16px ${c}30` }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color={c} fontSize={fontSize} fontWeight={font?.weight || 700} mode={tickerMode} />
      </div>
    ),
    "Minimal": (
      <div style={{ display: "flex", alignItems: "center", width: "100%", height: h, background: "rgba(0,0,0,0.88)", borderTop: "1px solid rgba(255,255,255,0.1)", gap: 12, padding: `0 ${isMobile ? 10 : 14}px` }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color={textColor} fontSize={fontSize} fontWeight={font?.weight || 400} separator="   ·   " mode={tickerMode} />
      </div>
    ),
    "Election": (
      <div style={{ display: "flex", alignItems: "stretch", width: "100%", height: h, background: "#0f172a", borderTop: `3px solid ${c}`, boxShadow: "0 -4px 20px rgba(239,68,68,0.3)" }}>
        ...
        <TickerDisplay text={displayText} speed={speed} color="#f8fafc" fontSize={fontSize} fontWeight={font?.weight || 800} separator="  ★  " mode={tickerMode} />
      </div>
    ),
  };
...
export default function BroadcastPage() {
  ...
  const hasOverlay = newsOverlay?.active || state?.adActive || state?.breakActive || state?.bgGradientActive || state?.qrActive;

  // Root background: always show the video output gradient behind everything.
  // The gradient runs permanently so that when the source drops, the coloured
  // background is immediately visible with no black flash.
  const rootBg = `linear-gradient(135deg, ${state?.videoOutputGradient1 ?? "#1a3a8f"}, ${state?.videoOutputGradient2 ?? "#00c853"})`;
  ...
  return (
    <div style={{
      ...
      {/* Watermark */}
      <div style={{
        position: "fixed", bottom: newsOverlay?.active ? 56 : 8, right: 8,
