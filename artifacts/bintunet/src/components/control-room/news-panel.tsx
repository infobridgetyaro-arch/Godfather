...
interface NewsOverlayState {
  ...
  ticker: { style: TickerStyle; direction: "left" | "right"; motion: string; speed: number; paused: boolean; separator: string };
...
function TickerScroll({ text, speed = 30, color = "#fff", fontSize = 13, fontWeight = 600, separator = "   ◆   " }: {
  ...
  // Stores measured layout so we can build a pixel-accurate CSS animation.
  ...
  useEffect(() => {
    ...
    function measure() {
      ...
      // Match backend: px/s proportional to canvas width; speed 1=slow 100=fast
      const pxPerSec = containerW / (speed * 0.4 + 4);
      // Single-pass distance: text enters from right edge, exits left edge
  ...
  // Hidden measurement render — keeps spanRef and containerRef mounted so
  // getBoundingClientRect() works on the first rAF tick.
  if (metrics === null) {
    return (
  ...
  // Single-pass animation matching the backend canvas renderer:
  //   0 %          — text starts just off the RIGHT edge (translateX = +containerW px)
  //   scrollPct %  — text has fully exited the LEFT edge (translateX = -spanW px)
  //                  timing switches to steps(1,end) → text stays hidden for the pause
  //   100 %        — jump back to +containerW px (both positions are off-screen; invisible)
...
// ── Constant gradient overlay (shared by all motion themes) ───────────────────

function GradientOverlay({ accent }: { accent: string }) {
  return (
    <div style={{
      position: "absolute", inset: 0, pointerEvents: "none", zIndex: 2,
      background: `linear-gradient(135deg, ${accent}28 0%, transparent 52%, ${accent}10 100%)`,
    }} />
  );
}
...
function ClockTick({ color = "#fff", fontSize = 11 }: { color?: string; fontSize?: number }) {
  ...
  return (
    <span style={{ fontVariantNumeric: "tabular-nums", fontFamily: "monospace", fontSize, color, fontWeight: 600, letterSpacing: "0.06em" }}>
      {time}
    </span>
  );
}

// ── Motion theme: Carousel ────────────────────────────────────────────────────
// Header + subheader + clock + looping slide-up animation

function MotionPreviewCarousel({ state }: { state: NewsOverlayState }) {
  const accent = state.customColors.primary;
  const msgs = state.tickerMessages.map(m => m.text);
  const items = msgs.length > 0 ? msgs : [
    state.headline.text || "Welcome to the live stream",
  ...
  return (
    <div style={{ position: "relative", height: 112, background: "linear-gradient(180deg, #08081a 0%, #03030f 100%)", overflow: "hidden" }}>
      <GradientOverlay accent={accent} />
      <style>{`
        @keyframes mp-carousel-up   { 0%{transform:translateY(110%);opacity:0} 14%{transform:translateY(0);opacity:1} 78%{transform:translateY(0);opacity:1} 100%{transform:translateY(-55%);opacity:0} }
        @keyframes mp-carousel-sub  { 0%{opacity:0;transform:translateY(6px)} 20%{opacity:1;transform:translateY(0)} 80%{opacity:1} 100%{opacity:0} }
        @keyframes mp-carousel-dot  { 0%,100%{opacity:1} 50%{opacity:0.2} }
      `}</style>

      {/* Top bar: LIVE badge + clock */}
      <div style={{ position: "relative", zIndex: 3, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 14px 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#ef4444", animation: "mp-carousel-dot 1.2s infinite" }} />
          <span style={{ fontSize: 9, fontWeight: 900, color: accent, letterSpacing: "0.14em", textTransform: "uppercase" }}>
            {state.liveBadge.label || "LIVE"}
          </span>
        </div>
        <ClockTick color="rgba(255,255,255,0.5)" fontSize={10} />
      </div>

      {/* Thin accent rule */}
      <div style={{ position: "absolute", left: 14, right: 14, top: 28, height: 1, background: `linear-gradient(90deg, ${accent}00, ${accent}bb, ${accent}00)`, zIndex: 3 }} />

      {/* Carousel headline — slides up on each tick */}
      <div style={{ position: "relative", zIndex: 3, padding: "10px 14px 3px", height: 42, overflow: "hidden", display: "flex", alignItems: "center" }}>
        <div
          key={animKey}
          style={{ animation: "mp-carousel-up 3.2s cubic-bezier(0.22,1,0.36,1) both", fontSize: 15, fontWeight: 800, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "100%" }}
        >
          {items[idx]}
        </div>
      </div>
      ...
      {/* Subheader */}
      <div style={{ position: "relative", zIndex: 3, padding: "0 14px" }}>
        <div
          key={`sub-${animKey}`}
          style={{ animation: "mp-carousel-sub 3.2s ease both", fontSize: 10, color: `${accent}cc`, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}
        >
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 4, background: `linear-gradient(90deg, transparent, ${accent}, ${accent}88, transparent)`, zIndex: 3 }} />
      <div style={{ position: "absolute", bottom: 4, left: 0, right: 0, height: 14, background: `linear-gradient(180deg, transparent, ${accent}1a)`, zIndex: 2 }} />

      {/* Dot indicators */}
      <div style={{ position: "absolute", bottom: 10, right: 14, zIndex: 4, display: "flex", gap: 4 }}>
        {items.map((_, i) => (
          <div key={i} style={{ width: 4, height: 4, borderRadius: "50%", background: i === idx ? accent : `${accent}44`, transition: "background 0.3s" }} />
        ))}
      </div>
    </div>
  );
...
// ── Motion theme: Stationary ──────────────────────────────────────────────────
// Frosted glass card, centered text, glowing border cross-fades

function MotionPreviewStationary({ state }: { state: NewsOverlayState }) {
  const accent = state.customColors.primary;
  const msgs = state.tickerMessages.map(m => m.text);
  const items = msgs.length > 0 ? msgs : [
    state.headline.text || "Welcome to the live stream",
  ...
  return (
    <div style={{ position: "relative", height: 76, background: "rgba(7,7,18,0.98)", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <GradientOverlay accent={accent} />
      <style>{`
        @keyframes mp-stat-fade    { 0%{opacity:0;transform:scale(0.96)} 16%{opacity:1;transform:scale(1)} 78%{opacity:1;transform:scale(1)} 100%{opacity:0;transform:scale(1.03)} }
        @keyframes mp-stat-glow    { 0%,100%{box-shadow:0 0 0 0 ${accent}00,inset 0 0 0 0 ${accent}00} 50%{box-shadow:0 0 18px 1px ${accent}40,inset 0 0 14px 0 ${accent}10} }
        @keyframes mp-stat-pulse   { 0%,100%{opacity:1} 50%{opacity:0.2} }
      `}</style>
      ...
      <div style={{
        ...
        animation: "mp-stat-glow 3.8s ease infinite",
        maxWidth: "88%", textAlign: "center",
      }}>
        {/* Top accent line */}
        <div style={{ position: "absolute", top: 0, left: 18, right: 18, height: 1, background: `linear-gradient(90deg, transparent, ${accent}cc, transparent)` }} />
        <div key={fadeKey} style={{ animation: "mp-stat-fade 3.8s ease both" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 280 }}>
            {items[idx]}
          </div>
        </div>
        {/* Bottom accent line */}
        <div style={{ position: "absolute", bottom: 0, left: 18, right: 18, height: 1, background: `linear-gradient(90deg, transparent, ${accent}66, transparent)` }} />
      </div>

      {/* LIVE badge top-left */}
      <div style={{ position: "absolute", zIndex: 4, top: 8, left: 12, display: "flex", alignItems: "center", gap: 5 }}>
        <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#ef4444", animation: "mp-stat-pulse 1.2s infinite" }} />
        <span style={{ fontSize: 8, fontWeight: 800, color: "rgba(255,255,255,0.4)", letterSpacing: "0.12em" }}>LIVE</span>
      </div>

      {/* Message count top-right */}
      <div style={{ position: "absolute", zIndex: 4, top: 8, right: 12 }}>
        <span style={{ fontSize: 8, color: `${accent}88`, fontFamily: "monospace" }}>{idx + 1}/{items.length}</span>
      </div>
    </div>
  );
}
...
// ── Motion theme: Flap ────────────────────────────────────────────────────────
// Split-flap departure-board aesthetic: amber tiles, monospace, clock row

function MotionPreviewFlap({ state }: { state: NewsOverlayState }) {
  const accent = state.customColors.primary;
  const msgs = state.tickerMessages.map(m => m.text);
  const items = msgs.length > 0 ? msgs : [state.headline.text || "LIVE STREAM NOW"];
  const [idx, setIdx] = useState(0);
  ...
  const raw = (items[idx] || "").toUpperCase().slice(0, 20).padEnd(20, " ");

  return (
    <div style={{ position: "relative", height: 84, background: "#0c0c0c", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "center", gap: 6 }}>
      <GradientOverlay accent={accent} />
      <style>{`
        @keyframes mp-flap { 0%{transform:rotateX(0deg);background:#1a1a1a} 40%{transform:rotateX(-88deg);background:#111} 41%{transform:rotateX(88deg);background:#222} 100%{transform:rotateX(0deg);background:#1a1a1a} }
      `}</style>

      {/* Label + clock row */}
      <div style={{ position: "relative", zIndex: 3, display: "flex", alignItems: "center", gap: 8, padding: "0 12px" }}>
        <span style={{ fontSize: 8, fontWeight: 700, color: "#f59e0b99", letterSpacing: "0.18em", textTransform: "uppercase" }}>DEPARTURES</span>
        <div style={{ flex: 1, height: 1, background: "rgba(245,158,11,0.18)" }} />
        <ClockTick color="#f59e0b" fontSize={9} />
      </div>

      {/* Tiles row */}
      <div style={{ position: "relative", zIndex: 3, display: "flex", gap: 2, padding: "0 12px", perspective: 500 }}>
        {raw.split("").map((ch, i) => (
          <div
            key={`${flapKey}-${i}`}
            style={{
              width: 13, height: 20, borderRadius: 2,
              background: "#1a1a1a", border: "1px solid #2a2a2a",
              display: "flex", alignItems: "center", justifyContent: "center",
              transformStyle: "preserve-3d",
              animation: `mp-flap 0.3s ease ${(i * 0.035).toFixed(3)}s both`,
              boxShadow: "0 2px 4px rgba(0,0,0,0.9), inset 0 1px 0 rgba(255,255,255,0.04)",
            }}
          >
            {/* Horizontal split line */}
            <div style={{ position: "absolute", left: 0, right: 0, top: "50%", height: 1, background: "#111", zIndex: 1 }} />
            <span style={{ fontSize: 10, fontWeight: 900, color: ch === " " ? "transparent" : "#f59e0b", fontFamily: "monospace", lineHeight: 1, position: "relative", zIndex: 2 }}>
              {ch}
            </span>
          </div>
        ))}
...
// ── Motion theme: Typewriter ──────────────────────────────────────────────────
// Terminal green-on-black, character-by-character reveal, blinking cursor, scan lines

function MotionPreviewTypewriter({ state }: { state: NewsOverlayState }) {
  const accent = state.customColors.primary;
  const msgs = state.tickerMessages.map(m => m.text);
  const items = msgs.length > 0 ? msgs : [state.headline.text || "Welcome to the live stream"];
  const [idx, setIdx] = useState(0);
  const [twKey, setTwKey] = useState(0);

  useEffect(() => {
    const charDur = Math.max(2800, items[idx].length * 65 + 1400);
    const t = setTimeout(() => {
      setIdx(i => (i + 1) % items.length);
      setTwKey(k => k + 1);
    }, charDur);
  ...
  const typeDurS = (charCount * 0.055).toFixed(2);

  return (
    <div style={{ position: "relative", height: 76, background: "#050f05", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "center" }}>
      <GradientOverlay accent={accent} />
      {/* CRT scan lines */}
      <div style={{ position: "absolute", inset: 0, zIndex: 1, pointerEvents: "none", backgroundImage: "repeating-linear-gradient(0deg, rgba(0,255,0,0.025) 0px, rgba(0,255,0,0.025) 1px, transparent 1px, transparent 4px)" }} />
      {/* Subtle vignette */}
      ...
      <style>{`
        @keyframes mp-tw-reveal-${twKey} { from{clip-path:inset(0 100% 0 0)} to{clip-path:inset(0 0% 0 0)} }
        @keyframes mp-cursor-blink       { 0%,100%{opacity:1} 50%{opacity:0} }
      `}</style>

      <div style={{ position: "relative", zIndex: 3, padding: "0 16px", display: "flex", flexDirection: "column", gap: 5 }}>
        {/* Prompt line */}
        <div style={{ fontSize: 8, color: "#22c55e55", fontFamily: "'Courier New', monospace", letterSpacing: "0.08em" }}>
          {`> BROADCAST LIVE`}
        </div>
        {/* Typed text + cursor */}
        <div style={{ display: "flex", alignItems: "center" }}>
          <span
            ...
            style={{
              ...
              animation: `mp-tw-reveal-${twKey} ${typeDurS}s steps(${charCount}, end) both`,
              whiteSpace: "nowrap", overflow: "hidden", maxWidth: "calc(100% - 12px)",
              textShadow: "0 0 8px #22c55e88",
            }}
          >
            {text}
          </span>
          <span style={{ fontSize: 13, color: "#4ade80", fontFamily: "monospace", animation: "mp-cursor-blink 0.75s step-end infinite", textShadow: "0 0 6px #22c55e" }}>
            ▋
          </span>
        </div>
      </div>
      ...
      {/* Bottom status bar */}
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 16, zIndex: 3, background: "#0a1a0a", borderTop: "1px solid #1a3a1a", display: "flex", alignItems: "center", padding: "0 14px", justifyContent: "space-between" }}>
        <span style={{ fontSize: 7, color: "#22c55e55", fontFamily: "monospace", letterSpacing: "0.1em" }}>STREAM ACTIVE</span>
        <span style={{ fontSize: 7, color: "#22c55e44", fontFamily: "monospace" }}>REC ●</span>
      </div>
    </div>
  );
}
...
// ── Motion theme: Wave ────────────────────────────────────────────────────────
// Characters with per-letter sinusoidal bounce, spectrum color shift

function MotionPreviewWave({ state }: { state: NewsOverlayState }) {
  const accent = state.customColors.primary;
  const msgs = state.tickerMessages.map(m => m.text);
  const [idx, setIdx] = useState(0);
  const [waveKey, setWaveKey] = useState(0);
  const items = msgs.length > 0 ? msgs : [state.headline.text || "LIVE WAVE"];
  ...
  const text = items[idx].slice(0, 30);

  return (
    <div style={{ position: "relative", height: 76, background: "linear-gradient(135deg, #0d0527 0%, #090118 55%, #050318 100%)", overflow: "hidden", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6 }}>
      <GradientOverlay accent={accent} />
      <style>{`
        @keyframes mp-wave-bob { 0%,100%{transform:translateY(0px)} 50%{transform:translateY(-8px)} }
        @keyframes mp-wave-fade-in { from{opacity:0;filter:blur(4px)} to{opacity:1;filter:blur(0)} }
      `}</style>

      {/* Wave characters */}
      <div
        key={waveKey}
        style={{ position: "relative", zIndex: 3, display: "flex", alignItems: "center", gap: 1, animation: "mp-wave-fade-in 0.5s ease both" }}
      >
        {text.split("").map((ch, i) => (
          <span
            key={i}
            style={{
              fontSize: 15, fontWeight: 800, display: "inline-block",
              color: `hsl(${(260 + i * 10) % 360}, 80%, 73%)`,
              animation: `mp-wave-bob 1.5s ease-in-out ${(i * 0.09).toFixed(2)}s infinite`,
              textShadow: `0 0 12px hsl(${(260 + i * 10) % 360}, 80%, 55%)`,
      ...
      {/* Bottom: LIVE + spectrum bar */}
      <div style={{ position: "relative", zIndex: 3, display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ width: 5, height: 5, borderRadius: "50%", background: "#a855f7", animation: "mp-wave-bob 1.2s ease-in-out infinite" }} />
        <span style={{ fontSize: 8, fontWeight: 800, color: "rgba(168,85,247,0.75)", letterSpacing: "0.12em" }}>LIVE</span>
        <div style={{ width: 60, height: 2, borderRadius: 1, background: "linear-gradient(90deg, #6366f1, #8b5cf6, #a855f7, #ec4899, #f43f5e)" }} />
      </div>
    </div>
  );
}

// ── Motion preview dispatcher ─────────────────────────────────────────────────

function MotionPreview({ state }: { state: NewsOverlayState }) {
  switch (state.ticker.motion ?? "Scroll") {
    case "Carousel":   return <MotionPreviewCarousel   state={state} />;
    case "Stationary": return <MotionPreviewStationary state={state} />;
    case "Flap":       return <MotionPreviewFlap       state={state} />;
    case "Typewriter": return <MotionPreviewTypewriter state={state} />;
    case "Wave":       return <MotionPreviewWave       state={state} />;
    default:           return null;
  }
}
...
function LivePreview({ state }: { state: NewsOverlayState }) {
  ...
  const motion = state.ticker.motion ?? "Scroll";
  ...
  return (
    <div style={{ borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)", background: "#111" }}>
      ...
      `}</style>
      <div style={{ padding: "6px 10px", fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.1em", background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 6 }}>
        <span>Live Preview —</span>
        {motion !== "Scroll"
          ? <span style={{ color: "rgba(102,126,234,0.8)", fontWeight: 700 }}>{motion}</span>
          : <span>{state.theme}</span>
        }
        {state.active && <span style={{ color: "#4ade80", marginLeft: 2 }}>● ACTIVE</span>}
        {motion === "Scroll" && animPreset !== "None" && <span style={{ color: "rgba(255,255,255,0.2)" }}>· {animPreset}</span>}
      </div>

      {/* Motion-specific themed preview */}
      {motion !== "Scroll" ? (
...
export function NewsPanel({ activeStreamCount }: { activeStreamCount: number }) {
  ...
  // ── Local drafts for free-text fields ─────────────────────────────────────
  // These fields used to patch the backend on every keystroke (debounced),
  // which meant a periodic WebSocket broadcast (ticker prune tick, headline
  // rotation, another operator's edit) landing mid-sentence would revert
  // whatever had been typed so far — the letters "sometimes got deleted".
  // Now typing only updates a local draft; the value is sent to the backend
  // once (on blur, Enter, or the Apply button), same as adding a ticker
  // message already worked.
  const [headlineDraft, setHeadlineDraft] = useState<string | null>(null);
  const [subHeadlineDraft, setSubHeadlineDraft] = useState<string | null>(null);
  ...
  // Top-level state keys with a locally-optimistic edit in flight (either
  // queued behind the debounce timer or waiting on the PATCH response).
  // While a key is "pending", incoming WebSocket state pushes (from the 30s
  // ticker-prune tick, headline auto-rotation, or another connected client)
  // must NOT overwrite it — otherwise a broadcast landing mid-edit reverts
  // whatever the user just typed/dragged, which is exactly what caused
  // controls to "sometimes work, sometimes not" and typed text to be
  // silently deleted.
      ...
      try {
        ...
        if (msg.type === "news-overlay" && msg.data) {
          setState(prev => {
            ...
            // Merge the broadcast in, but keep the locally-edited value for any
            // key that's still pending so we don't clobber an in-progress edit.
            const merged: NewsOverlayState = { ...msg.data };
            for (const k of pendingKeysRef.current) {
              if (k in prev) (merged as any)[k] = (prev as any)[k];
            }
            return merged;
          });
        }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {tab === "ticker" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <Section label="Motion Style">
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", lineHeight: 1.4 }}>
                Controls <em>how</em> the text moves. Use Scroll for classic broadcast look, or pick a creative motion.
              </div>
              {[
                { v: "Scroll",      label: "⟶ Scroll",      desc: "Classic right-to-left broadcast scroll" },
                { v: "Stationary",  label: "◼ Stationary",  desc: "Fixed text, messages cross-fade every 6s" },
                { v: "Flap",        label: "⟳ Flap",        desc: "Split-flap departure board animation" },
                { v: "Typewriter",  label: "▌ Typewriter",  desc: "Types in character by character" },
                { v: "Wave",        label: "〜 Wave",        desc: "Characters bob on a sinusoidal wave" },
                { v: "Carousel",    label: "↑ Carousel",    desc: "Messages slide up one at a time" },
              ].map(({ v, label, desc }) => {
                const active = (state.ticker.motion ?? "Scroll") === v;
                return (
                  <button key={v}
                    onClick={() => patch({ ticker: { ...state.ticker, motion: v } })}
                    style={{
                      display: "flex", alignItems: "center", gap: 8,
                  ...
                  >
                    <span style={{ fontSize: 11, fontWeight: 700, minWidth: 88 }}>{label}</span>
          ...
          <Section label="Headline Text">
            ...
            {headlineDraft !== null && headlineDraft !== state.headline.text && (
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <button
                  onClick={() => { patch({ headline: { ...state.headline, text: headlineDraft } }); setHeadlineDraft(null); }}
                  style={{ padding: "5px 12px", borderRadius: 7, background: "rgba(102,126,234,0.2)", border: "1px solid rgba(102,126,234,0.4)", color: "#a5b4fc", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                  Apply
                </button>
                <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>Unsaved — press Enter or Apply to send</span>
              </div>
            )}
          </Section>

          <Section label="Sub-Headline">
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginBottom: 5 }}>
              Shown below the main headline in Stationary, Flap, Typewriter, Wave, and Carousel modes
            </div>
            <textarea
              value={subHeadlineDraft ?? (state.headline.subHeadline ?? "")}
              onChange={e => setSubHeadlineDraft(e.target.value)}
              onBlur={() => {
                const val = subHeadlineDraft;
                if (val === null || val === (state.headline.subHeadline ?? "")) { setSubHeadlineDraft(null); return; }
                patch({ headline: { ...state.headline, subHeadline: val } });
                setSubHeadlineDraft(null);
              }}
              placeholder="e.g. Follow us @channel · Subscribe for more"
              rows={2}
              style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "7px 12px", color: "rgba(255,255,255,0.8)", fontSize: 12, outline: "none", resize: "vertical", fontFamily: "inherit", boxSizing: "border-box" }}
              onKeyDown={e => {
                if (e.key === "Enter" && !e.shiftKey) {
                  e.preventDefault();
                  (e.target as HTMLTextAreaElement).blur();
            ...
            />
            {subHeadlineDraft !== null && subHeadlineDraft !== (state.headline.subHeadline ?? "") && (
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <button
                  onClick={() => { patch({ headline: { ...state.headline, subHeadline: subHeadlineDraft } }); setSubHeadlineDraft(null); }}
                  style={{ padding: "5px 12px", borderRadius: 7, background: "rgba(102,126,234,0.2)", border: "1px solid rgba(102,126,234,0.4)", color: "#a5b4fc", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                  Apply
                </button>
                <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>Press Enter or Apply to send</span>
              </div>
            )}
          </Section>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <Section label="Breaking News Text">
            ...
            {breakingDraft !== null && breakingDraft !== state.breakingNews.text && (
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <button
                  onClick={() => { patch({ breakingNews: { ...state.breakingNews, text: breakingDraft } }); setBreakingDraft(null); }}
                  style={{ padding: "5px 12px", borderRadius: 7, background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.4)", color: "#f87171", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                  Apply
                </button>
                <span style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>Unsaved — press Enter or Apply to send</span>
              </div>
          ...
          <Section label="Live Badge">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <input
                ...
                onKeyDown={e => { if (e.key === "Enter") (e.target as HTMLInputElement).blur(); }}
                placeholder="Badge label (e.g. LIVE) — applies on blur or Enter"
                style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 7, padding: "5px 10px", color: "#fff", fontSize: 11, outline: "none", flex: 1, minWidth: 80 }} />
