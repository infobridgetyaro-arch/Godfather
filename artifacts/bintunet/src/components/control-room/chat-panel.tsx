...
function getNameColor(_msg: ChatMessage): string {
  // White text for all roles — clean, professional broadcast look
  return "#ffffff";
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ minHeight: 200, maxHeight: pinnedMessage ? 320 : 380, overflowY: "auto", display: "flex", flexDirection: "column", gap: 0, padding: "6px 0" }}>
        {messages.length === 0 && (
          <div style={{ color: "rgba(56,189,248,0.4)", fontSize: 12, textAlign: "center", padding: "60px 0", display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
        ...
        return (
          <div key={msg.id} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.88)", wordBreak: "break-word", lineHeight: 1.55, overflowWrap: "anywhere" }}>{msg.text}</div>
            </div>
          </div>
        );
      })}
...
function CompactChat({ messages }: { messages: QueuedMessage[] }) {
  ...
  return (
    <div style={{ minHeight: 200, maxHeight: 380, overflowY: "auto" }}>
      <div style={{ padding: "5px 12px 5px 10px", borderBottom: "1px solid rgba(56,189,248,0.1)", display: "flex", alignItems: "center", gap: 6, background: "linear-gradient(90deg, rgba(14,165,233,0.10) 0%, rgba(99,102,241,0.06) 100%)" }}>
        <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#ef4444", animation: "cp-pulse 1.2s infinite" }} />
        <span style={{ fontSize: 9, color: "rgba(56,189,248,0.8)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", flex: 1 }}>Live Chat</span>
        <span style={{ fontSize: 9, color: "rgba(255,255,255,0.35)", fontWeight: 600 }}>{latest.length}</span>
      </div>
      {latest.length === 0 && <div style={{ color: "rgba(255,255,255,0.25)", fontSize: 12, textAlign: "center", padding: "40px 0" }}>No messages yet…</div>}
      {latest.map((msg) => {
        const nameColor = getNameColor(msg);
        return (
          <div key={msg.id} style={{
            ...
            background: msg.entering
              ? "linear-gradient(90deg, rgba(14,165,233,0.10) 0%, rgba(99,102,241,0.06) 100%)"
              : "transparent",
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <span style={{ fontSize: 11, color: "rgba(255,255,255,0.85)", wordBreak: "break-word", overflowWrap: "anywhere" }}>{msg.text}</span>
            </div>
            {msg.superChatAmount && (
              <span style={{ fontSize: 9, fontWeight: 800, color: "#fcd34d", flexShrink: 0, marginTop: 3 }}>★ {msg.superChatAmount}</span>
            )}
          </div>
        );
      })}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {activeStreamCount === 0 ? (
        <div style={{
          ...
          <MessageSquare size={20} style={{ color: "rgba(56,189,248,0.3)" }} />
          <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>Chat appears when a stream is live</span>
        </div>
      ) : (
        <div style={{
          borderRadius: 14,
          border: `1px solid ${accent}28`,
          background: `linear-gradient(160deg, rgba(10,18,42,0.92) 0%, rgba(18,8,38,0.88) 50%, rgba(8,16,36,0.92) 100%)`,
          overflow: "hidden",
      ...
      {currentStyle === "Live Feed" && activeStreamCount > 0 && (
        <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", textAlign: "center", letterSpacing: "0.02em" }}>
