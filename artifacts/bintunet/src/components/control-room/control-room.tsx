...
interface BroadcastState {
  ...
  // News ticker loop
  newsTickerLoopSecs: number;
  // Logo overlay (single – kept for backward compat with renderer)
  logoActive: boolean;
  logoUrl: string;
  logoX: number;
  logoY: number;
  ...
  logoBgColor: string;
  logoBgShape: "circle" | "rounded" | "square";
  logoAnimation: "None" | "Fade" | "Slide In" | "Zoom" | "Bounce" | "Spin";
  logoAnimGap: number;
  // Multi-logo overlay
  ...
  disclaimerSoundUrl: string;
  disclaimerStyle: "Warning" | "Legal" | "Copyright" | "Viewer Advisory";
}
...
interface ControlRoomProps {
  ...
  onForceRefreshChat?: () => void;
}

type Tab = "news" | "social" | "ads" | "break" | "chat" | "stats" | "subs" | "bg" | "alerts" | "mic" | "qr" | "donate" | "screen" | "music" | "stage" | "yt-api" | "system";
...
const TABS: { id: Tab; label: string; icon: React.ReactNode; accent: string }[] = [
  ...
  { id: "social", label: "Social",    icon: <span style={{ fontSize: 11 }}>◎</span>, accent: "#ec4899" },
  ...
  { id: "system", label: "System",   icon: <Server size={13} />,                      accent: "#38bdf8" },
];
...
const LOGO_ANIMATIONS   = ["None", "Fade", "Slide In", "Zoom", "Bounce", "Spin"] as const;
const LOGO_BG_SHAPES    = ["circle", "rounded", "square"] as const;

/** Best-effort rgba()/hex → hex, for feeding an <input type="color"> which only accepts #rrggbb. */
...
async function sendBroadcastPatch(patch: Partial<BroadcastState>) {
  try {
...
// Per-field debounce buffers, keyed by the sorted set of field names in the
// patch. A drag-driven slider (size, ticker loop pause, etc.) fires onChange
// on every pixel of movement — without coalescing, each tick sends its own
// POST /api/broadcast, and slow/out-of-order responses race each other so
// the server can end up applying an intermediate drag value instead of the
// final one, making the slider "sometimes not take effect". Debouncing per
// field lets the UI update instantly (via optimistic local state) while only
// the *last* value in a burst is actually sent to the server.
const _broadcastDebounceTimers: Record<string, ReturnType<typeof setTimeout>> = {};
const _broadcastDebounceBuffers: Record<string, Partial<BroadcastState>> = {};

async function pushBroadcast(patch: Partial<BroadcastState>, debounceMs = 0) {
  if (debounceMs <= 0) {
    await sendBroadcastPatch(patch);
    return;
  }
  const key = Object.keys(patch).sort().join(",");
  _broadcastDebounceBuffers[key] = { ..._broadcastDebounceBuffers[key], ...patch };
  if (_broadcastDebounceTimers[key]) clearTimeout(_broadcastDebounceTimers[key]);
  _broadcastDebounceTimers[key] = setTimeout(() => {
    const buffered = _broadcastDebounceBuffers[key];
    delete _broadcastDebounceBuffers[key];
    delete _broadcastDebounceTimers[key];
    if (buffered) sendBroadcastPatch(buffered);
  }, debounceMs);
}
...
function SizeSlider({ value, onChange, accent, max = 200 }: {
  value: number;
...
// Apply Style to Stage button with "Applied ✓" flash feedback
function ApplyStageButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${applied ? "rgba(74,222,128,0.5)" : "rgba(52,211,153,0.45)"}`,
        background: applied
        ...
        color: applied ? "#4ade80" : "#34d399",
      }}
    >
      {applied ? "Applied ✓" : "Apply Style to Stage"}
    </button>
  );
}
...
export function ControlRoom({ streams, streamStats, streamChat, streamProcStats = {}, onForceRefreshChat }: ControlRoomProps) {

  // Force-refresh chat: call the backend refresh endpoint for each YouTube stream
  // so the server polls YouTube API immediately, then trigger local re-fetch.
  const [isRefreshingChat, setIsRefreshingChat] = useState(false);
  const [chatRefreshMessage, setChatRefreshMessage] = useState<string | null>(null);

  const handleForceRefreshChat = useCallback(async () => {
    if (isRefreshingChat) return;
    setIsRefreshingChat(true);
    setChatRefreshMessage("Checking YouTube for live status…");

    const ytStreams = streams.filter(
      (s) => s.youtubeSourceUrl || (s as any).youtubeChannelId || (s as any).sourceType === "youtube"
    );
    const ids = ytStreams.length > 0 ? ytStreams.map((s) => s.id) : streams.map((s) => s.id);

    try {
      const results = await Promise.all(
        ids.map(async (id) => {
          try {
            const res = await fetch(`/api/streams/${id}/chat/refresh`, {
              method: "POST",
              credentials: "include",
            });
      ...
      const live = results.find((r) => r && r.isLive);
      if (live) {
        const viewers = live.viewers ? `${live.viewers} watching` : "viewer count unavailable";
        setChatRefreshMessage(`🔴 Channel is live — ${viewers}`);
      } else {
        const failed = results.find((r) => r && r.error && r.error !== "not_live");
        if (failed) {
          setChatRefreshMessage(`Couldn't verify live status: ${failed.error}`);
        } else {
          setChatRefreshMessage("Channel is not live right now");
        }
      }
    } catch (e: any) {
      setChatRefreshMessage(`Refresh failed: ${e?.message ?? "unknown error"}`);
    } finally {
      setIsRefreshingChat(false);
      setTimeout(() => setChatRefreshMessage(null), 6000);
    }
  }, [streams, onForceRefreshChat, isRefreshingChat]);
  ...
  const [bs, setBs] = useState<BroadcastState>({
    ...
    chatStyle: "TV",
    statsActive: false, statsStyle: "TV", statsPosition: { x: 2, y: 2 },
    ...
    disclaimerText: "⚠️ Some music in this stream may be protected by copyright. This content is intended for viewers 13 and older. Viewer discretion is advised.",
    disclaimerSoundUrl: "",
  ...
  const newsPosDebRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const startScreenShare = useCallback(async () => {
    ...
    try {
      ...
      // Some Android Chrome builds expose getDisplayMedia() (for spec
      // compliance) but silently never resolve/reject the promise and never
      // hand off to the OS screen-recorder — no dialog, no error, nothing.
      // This happens when the device/Chrome build lacks MediaProjection
      // wiring for web screen capture. Without a timeout the UI would hang
      // on "Connecting..." forever with zero feedback. Race a 10s timeout so
      // we can surface a concrete, actionable message instead.
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => {
            reject(Object.assign(
              new Error(
                isAndroid
                  ? `No screen-capture prompt appeared after 10s. Your device's Chrome build` +
                    (chromeVersion ? ` (v${chromeVersion})` : "") +
                    ` doesn't support in-browser screen capture (Android's MediaProjection handoff isn't wired up on this device/ROM). ` +
                    `Try: 1) Update Chrome from the Play Store, 2) Make sure you're using Chrome (not Samsung Internet, Brave, or an in-app browser), ` +
                    `3) If it still doesn't prompt, this device/browser combo doesn't support web screen capture — use a desktop browser (Chrome/Firefox/Edge) instead.`
                  : "No screen-capture prompt appeared after 10s. Your browser may not support this feature — try Chrome, Firefox, or Edge on desktop."
              ),
              { name: "NoPromptTimeout" }
            ));
          }, 10000)
        ),
      ]);
      ...
      const msg =
        ...
        e?.name === "NoPromptTimeout" ? e.message :
        `Error: ${e?.message ?? e}`;
      setScreenError(msg);
      setScreenConnecting(false);
    }
  ...
  // Both requests are fired in parallel (they're independent network calls —
  // there's no reason to make the dashboard wait for the first to finish
  // before starting the second). Ordering only matters for how the *results*
  // are applied: broadcast state is applied first, then overlay position on
  // top, so the API-driven position always wins and is never overwritten by
  // stale legacy values.
  useEffect(() => {
    let cancelled = false;
    const broadcastReq = fetch("/api/broadcast", { credentials: "include" }).catch(() => null);
    const newsOverlayReq = fetch("/api/news-overlay", { credentials: "include" }).catch(() => null);
    (async () => {
      const br = await broadcastReq;
      if (!cancelled && br?.ok) {
        try {
      ...
      // Apply API news-overlay position after broadcast state so it is never
      // overwritten by a legacy newsPosition value from /api/broadcast.
      const nr = await newsOverlayReq;
      if (!cancelled && nr?.ok) {
        try {
          const d = await nr.json();
          if (d?.layout?.position) {
            setBs((prev) => ({
              ...prev,
              newsPosition: d.layout.position,
              mobileNewsPosition: d.layout.position,
            }));
          }
        } catch {}
      }
  ...
  // Clear pending news-position debounce on unmount so no stale PATCH fires
  useEffect(() => {
    return () => { if (newsPosDebRef.current) clearTimeout(newsPosDebRef.current); };
  }, []);
  ...
  const update = useCallback((patch: Partial<BroadcastState>, debounceMs = 0) => {
    setBs((prev) => ({ ...prev, ...patch }));
    pushBroadcast(patch, debounceMs);
  }, []);
  ...
  const startMic = useCallback(async () => {
    ...
    try {
      ...
      try {
        stream = await navigator.mediaDevices.getUserMedia({
          audio: {
            ...
            autoGainControl: false,   // disabled — our compressor handles levelling
            channelCount: 1,
            sampleRate: 48000,        // broadcast standard; eliminates SRC in FFmpeg
          },
          video: false,
        });
      } catch (e: any) {
      ...
      const ctx = new AudioContext({ sampleRate: 48000 }); // broadcast standard — no SRC in FFmpeg
      ...
      // High-pass filter: cut rumble / fan noise / mic handling below 100 Hz.
      // 2nd-order Butterworth (Q = 0.707) — maximally flat passband, no ringing.
      ...
      // Dynamics compressor: broadcast-style levelling.
      //
      // threshold -18 dB  — start compressing at a moderate speaking level
      // knee       8 dB   — soft knee keeps the onset transparent
      // ratio      3:1    — gentle ratio; 4:1 caused audible "pumping" on plosives
      // attack    3 ms    — fast enough to catch transients before they clip
      // release  200 ms   — slow release = smooth, natural decay; prevents
      //                     gain-pumping between words (the "radio duck" effect)
  ...
  const setPos = (base: "statsPosition" | "subsPosition" | "chatBurnPosition" | "newsPosition" | "adPosition" | "guestPosition" | "subChartPosition") =>
    (p: OverlayPosition) => update({ [posKey(base)]: p } as any);
        ...
        <div style={{ padding: "16px 18px", animation: "cr-slide-down 0.25s ease forwards" }}>
          ...
          {activeTab === "stats" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {/* Style description */}
              <div style={{ padding: "8px 12px", borderRadius: 7, background: "linear-gradient(135deg, rgba(167,139,250,0.07) 0%, rgba(99,102,241,0.04) 100%)", border: "1px solid rgba(167,139,250,0.1)", fontSize: 11, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>
              ...
              <div style={{ padding: "9px 14px", borderRadius: 8, background: "linear-gradient(135deg, rgba(167,139,250,0.09) 0%, rgba(99,102,241,0.04) 100%)", border: "1px solid rgba(167,139,250,0.18)", fontSize: 11, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>
          ...
          {activeTab === "subs" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ padding: "8px 12px", borderRadius: 7, background: "linear-gradient(135deg, rgba(204,0,1,0.07) 0%, rgba(139,0,0,0.04) 100%)", border: "1px solid rgba(204,0,1,0.14)", fontSize: 11, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>
              ...
              <div style={{ padding: "9px 14px", borderRadius: 8, background: "linear-gradient(135deg, rgba(204,0,1,0.09) 0%, rgba(139,0,0,0.04) 100%)", border: "1px solid rgba(204,0,1,0.18)", fontSize: 11, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>
                Style changes are staged — tap <strong style={{ color: "#fc8181" }}>Go Live</strong> to apply them with a 3-second countdown.
              </div>
            </div>
          )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {allChatMessages.length === 0 ? (
                <div style={{
                  ...
                  <span style={{ color: "rgba(255,255,255,0.45)", fontSize: 12, lineHeight: 1.5, flex: 1, minWidth: 0 }}>
                    Waiting for YouTube live chat…<br />
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.25)" }}>Auto-refreshes every 10 s</span>
                  </span>
                  <button
                    onClick={handleForceRefreshChat}
                    disabled={isRefreshingChat}
                    style={{
                      flexShrink: 0, padding: "4px 10px", borderRadius: 6,
                      background: "rgba(52,211,153,0.12)", border: "1px solid rgba(52,211,153,0.3)",
                      color: "#34d399", fontSize: 11, fontWeight: 600,
                      cursor: isRefreshingChat ? "default" : "pointer",
                      whiteSpace: "nowrap", opacity: isRefreshingChat ? 0.6 : 1,
                      display: "flex", alignItems: "center", gap: 5,
                    }}
                  >
                    <RefreshCw size={11} style={isRefreshingChat ? { animation: "cr-spin 1s linear infinite" } : undefined} />
                    {isRefreshingChat ? "Refreshing…" : "Force Refresh"}
                  </button>
                </div>
              ) : (
                <div style={{
                  ...
                  background: "linear-gradient(160deg, rgba(10,20,38,0.92) 0%, rgba(18,10,35,0.88) 100%)",
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 6 }}>
                    <button
                      ...
                      style={{
                        ...
                        color: "#34d399", fontSize: 11, fontWeight: 600,
                        cursor: isRefreshingChat ? "default" : "pointer",
                        opacity: isRefreshingChat ? 0.6 : 1,
                        display: "flex", alignItems: "center", gap: 5,
                      }}
                    >
                      <RefreshCw size={11} style={isRefreshingChat ? { animation: "cr-spin 1s linear infinite" } : undefined} />
              ...
              {chatRefreshMessage && (
                <div style={{
                  ...
                  background: isRefreshingChat ? "rgba(255,255,255,0.05)" : "rgba(52,211,153,0.08)",
                  border: `1px solid ${isRefreshingChat ? "rgba(255,255,255,0.12)" : "rgba(52,211,153,0.25)"}`,
                  color: isRefreshingChat ? "rgba(255,255,255,0.55)" : "#34d399",
                  fontSize: 11.5, fontWeight: 600,
                  display: "flex", alignItems: "center", gap: 7,
                }}>
                  {isRefreshingChat && <RefreshCw size={11} style={{ animation: "cr-spin 1s linear infinite" }} />}
                  {chatRefreshMessage}
                </div>
              )}
              ...
              <div style={{ padding: "12px 16px", borderRadius: 10, background: "linear-gradient(135deg, rgba(52,211,153,0.10) 0%, rgba(16,185,129,0.05) 100%)", border: "1px solid rgba(52,211,153,0.22)" }}>
              ...
              <ApplyStageButton onClick={() => update({ chatStyle: bs.chatStyle })} />
              ...
              <div style={{ padding: "10px 14px", borderRadius: 8, background: "linear-gradient(135deg, rgba(52,211,153,0.09) 0%, rgba(16,185,129,0.04) 100%)", border: "1px solid rgba(52,211,153,0.18)", fontSize: 11, color: "rgba(255,255,255,0.6)", lineHeight: 1.6 }}>
          ...
          {activeTab === "news" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ padding: "10px 12px", background: "linear-gradient(135deg, rgba(102,126,234,0.08) 0%, rgba(99,102,241,0.04) 100%)", borderRadius: 10, border: "1px solid rgba(102,126,234,0.15)", display: "flex", flexDirection: "column", gap: 10 }}>
                <PositionSliders
                  pos={getPos("newsPosition")}
                  label="Position"
                  accent="#667eea"
                  onChange={(p) => {
                    // pushBroadcast is the single source of truth for position.
                    // The server's /api/broadcast handler sees "newsPosition" in
                    // the body and calls updateStreamOverlays directly — so the
                    // canvas renderer gets the new XY without touching newsActive.
                    setPos("newsPosition")(p);
                  }}
                />
                <SizeSlider value={bs.newsScale} onChange={(v) => update({ newsScale: v }, 120)} accent="#667eea" max={300} />
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <span style={{ fontSize: 11, color: "#a5b4fc", fontWeight: 600 }}>
                      {bs.newsTickerLoopSecs}s
                    </span>
                  </div>
                  <input
                    type="range"
                    min={5} max={60} step={5}
                    value={bs.newsTickerLoopSecs}
                    onChange={(e) => {
                      const v = Number(e.target.value);
                      localUpdate({ newsTickerLoopSecs: v });
                      update({ newsTickerLoopSecs: v }, 120);
                    }}
                    style={{ width: "100%", accentColor: "#667eea", cursor: "pointer" }}
                  />
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)" }}>
                    How long the ticker pauses after each full loop (5s – 60s)
                  </div>
                </div>
              </div>
              <Suspense fallback={<PanelSpinner />}>
          ...
          {activeTab === "ads" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {/* ── DISCLAIMER ── */}
              <div style={{ marginTop: 4, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 12, display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 16 }}>⚠️</span>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.7)", letterSpacing: "0.04em" }}>Disclaimer</span>
                  <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 99, background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.25)", color: "#fbbf24" }}>
                    Copyright &amp; Viewer Advisory
                  </span>
                </div>

                {/* Style pills */}
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                  {(["Warning", "Legal", "Copyright", "Viewer Advisory"] as const).map((s) => (
                    <button key={s} onClick={() => localUpdate({ disclaimerStyle: s })}
                      style={{
                        padding: "4px 10px", borderRadius: 20, fontSize: 10, fontWeight: 600, cursor: "pointer",
                        border: `1px solid ${bs.disclaimerStyle === s ? "#fbbf24" : "rgba(255,255,255,0.1)"}`,
                        background: bs.disclaimerStyle === s ? "rgba(251,191,36,0.12)" : "rgba(255,255,255,0.03)",
                        color: bs.disclaimerStyle === s ? "#fcd34d" : "rgba(255,255,255,0.4)",
                ...
                {/* Disclaimer text paragraph */}
                <textarea
                  value={bs.disclaimerText}
                  onChange={(e) => localUpdate({ disclaimerText: e.target.value })}
                  rows={3}
                  placeholder="Enter disclaimer paragraph…"
                  style={{
                    width: "100%", padding: "8px 11px", borderRadius: 8, fontSize: 11,
                ...
                {/* Copyright-free sound URL */}
                <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", display: "flex", alignItems: "center", gap: 5 }}>
                    <span>🎵</span>
                    <span>Background music (YouTube copyright-free)</span>
                  </div>
                  <input
                    type="text"
                    value={bs.disclaimerSoundUrl}
                    onChange={(e) => localUpdate({ disclaimerSoundUrl: e.target.value })}
                    placeholder="Paste YouTube Audio Library URL…"
                    style={{
                      width: "100%", padding: "7px 11px", borderRadius: 8, fontSize: 11,
                      background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)",
                      color: "#fff", outline: "none", boxSizing: "border-box",
                    }}
                  />
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,0.25)", lineHeight: 1.5 }}>
                    Find royalty-free tracks at{" "}
                    <a href="https://studio.youtube.com/channel/audio" target="_blank" rel="noopener noreferrer"
                      style={{ color: "#93c5fd", textDecoration: "none" }}>
                      YouTube Audio Library
                    </a>
                    . Audio plays in your browser while disclaimer is live.
                  </div>
                </div>

                {/* Audio preview — plays in browser when disclaimerSoundUrl is set & active */}
                {bs.disclaimerActive && bs.disclaimerSoundUrl && (() => {
                  const ytMatch = bs.disclaimerSoundUrl.match(/(?:v=|youtu\.be\/|\/shorts\/|\/embed\/|\/live\/)([a-zA-Z0-9_-]{11})/);
                  return ytMatch?.[1] ? (
                    <iframe
                      key={bs.disclaimerSoundUrl}
                      src={`https://www.youtube.com/embed/${ytMatch[1]}?autoplay=1&controls=0&loop=1&playlist=${ytMatch[1]}&mute=0`}
                      allow="autoplay"
                      style={{ width: "100%", height: 0, border: "none", position: "absolute", opacity: 0, pointerEvents: "none" }}
                      title="Disclaimer background audio"
                    />
                  ) : null;
                })()}

                {/* Disclaimer preview card */}
                {(bs.disclaimerText || bs.disclaimerStyle) && (
                  <div style={{
                    padding: "10px 12px", borderRadius: 8,
                    background: bs.disclaimerStyle === "Warning"
                      ? "linear-gradient(135deg,rgba(245,158,11,0.18),rgba(217,119,6,0.08))"
                      : bs.disclaimerStyle === "Copyright"
                        ? "linear-gradient(135deg,rgba(239,68,68,0.18),rgba(185,28,28,0.08))"
                        : bs.disclaimerStyle === "Legal"
                          ? "linear-gradient(135deg,rgba(99,102,241,0.18),rgba(67,56,202,0.08))"
                          : "linear-gradient(135deg,rgba(20,184,166,0.18),rgba(15,118,110,0.08))",
                    border: bs.disclaimerStyle === "Warning"
                    ...
                    <div style={{ fontSize: 10, fontWeight: 700, color: "#fcd34d", marginBottom: 4 }}>
                      {bs.disclaimerStyle === "Warning" ? "⚠️ WARNING" :
                       bs.disclaimerStyle === "Copyright" ? "©️ COPYRIGHT NOTICE" :
                       bs.disclaimerStyle === "Legal" ? "⚖️ LEGAL DISCLAIMER" :
                       "👁️ VIEWER ADVISORY"}
                    </div>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.65)", lineHeight: 1.5, whiteSpace: "pre-wrap" }}>
                      {bs.disclaimerText || "No disclaimer text set."}
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <ToggleButton
                    active={bs.disclaimerActive}
                    accent="#fbbf24"
                    countdownSecs={countdowns["disclaimer"]}
                    onCancel={() => cancelGoLive("disclaimer")}
                    onActivate={() => goLive("disclaimer", {
                      disclaimerActive: true,
                      disclaimerText: bs.disclaimerText,
                      disclaimerStyle: bs.disclaimerStyle,
                      disclaimerSoundUrl: bs.disclaimerSoundUrl,
                    })}
                    onDeactivate={() => stopOverlay({ disclaimerActive: false })}
                  />
                  <LiveBadge label={`${bs.disclaimerStyle} disclaimer`} active={bs.disclaimerActive} accent="#fbbf24" />
                </div>
              </div>
          ...
          {activeTab === "bg" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {/* ── LOGO OVERLAYS ── */}
              <SectionDivider label="Logo Overlays" />
              <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, lineHeight: 1.6 }}>
                Place logos on the stream. Upload PNGs with transparent backgrounds. Each logo has independent XY position and size (up to 300%).
              </div>

              {/* Per-logo cards */}
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {(bs.logos ?? []).map((logo, idx) => (
                  <div key={logo.id} style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: "#fda4af", textTransform: "uppercase", letterSpacing: "0.07em" }}>
                        Logo {idx + 1}
                      </span>
                      <button
                        onClick={() => {
                          const updated = (bs.logos ?? []).filter((l) => l.id !== logo.id);
                          localUpdate({ logos: updated });
                          update({ logos: updated });
                        }}
                        style={{ padding: "2px 9px", borderRadius: 6, border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.08)", color: "#f87171", cursor: "pointer", fontSize: 11, fontWeight: 600 }}
                      >
                        ✕ Remove
                      </button>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      ...
                      <label style={{
                        ...
                        borderRadius: 8, border: "1px dashed rgba(251,113,133,0.4)",
                        background: "rgba(251,113,133,0.05)", cursor: "pointer", color: "#fda4af", fontSize: 11, fontWeight: 600,
                      }}>
                        <Upload size={11} />
                        {logo.url ? "Replace" : "Upload PNG/SVG"}
                        <input
                          type="file"
                          accept="image/*"
                          style={{ display: "none" }}
                          onChange={(e) => {
                            ...
                            reader.onload = (ev) => {
                              ...
                              const updated = (bs.logos ?? []).map((l) => l.id === logo.id ? { ...l, url } : l);
                              localUpdate({ logos: updated });
                              update({ logos: updated });
                            };
                    ...
                    {/* X / Y / Size sliders */}
                    {[
                      { key: "x" as const, label: "X", min: 0, max: 100, step: 1, fmt: (v: number) => `${v}%` },
                      { key: "y" as const, label: "Y", min: 0, max: 90,  step: 1, fmt: (v: number) => `${v}%` },
                    ].map(({ key, label, min, max, step, fmt }) => (
                      <div key={key} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{label}</span>
                          <span style={{ fontSize: 10, color: "#fda4af", fontWeight: 600 }}>{fmt(logo[key])}</span>
                        </div>
                        <input
                          type="range" min={min} max={max} step={step} value={logo[key]}
                          onChange={(e) => {
                            const updated = (bs.logos ?? []).map((l) => l.id === logo.id ? { ...l, [key]: Number(e.target.value) } : l);
                            localUpdate({ logos: updated });
                            update({ logos: updated });
                          }}
                    ...
                    {/* Size — displayed as percentage (100% = 120 px) */}
                    <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.07em" }}>Size</span>
                        <span style={{ fontSize: 10, color: "#fda4af", fontWeight: 600 }}>{Math.round((logo.size / 120) * 100)}%</span>
                      </div>
                      <input
                        type="range" min={40} max={360} step={10} value={logo.size}
                        onChange={(e) => {
                          const updated = (bs.logos ?? []).map((l) => l.id === logo.id ? { ...l, size: Number(e.target.value) } : l);
                          localUpdate({ logos: updated });
                          update({ logos: updated });
                        }}
                        style={{ accentColor: "#fb7185", cursor: "pointer" }}
                      />
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ fontSize: 9, color: "rgba(255,255,255,0.22)" }}>33%</span>
                        <span style={{ fontSize: 9, color: "rgba(255,255,255,0.22)" }}>100%</span>
                        <span style={{ fontSize: 9, color: "rgba(255,255,255,0.22)" }}>200%</span>
                        <span style={{ fontSize: 9, color: "rgba(255,255,255,0.22)" }}>300%</span>
                      </div>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      ...
                      <StylePills
                        styles={LOGO_ANIMATIONS}
                        current={logo.animation ?? "Fade"}
                        accent="#fb7185"
                        onSelect={(v) => {
                          const updated = (bs.logos ?? []).map((l) => l.id === logo.id ? { ...l, animation: v as typeof LOGO_ANIMATIONS[number] } : l);
                          localUpdate({ logos: updated });
                          update({ logos: updated });
                        }}
                      />
                      {(logo.animation ?? "Fade") !== "None" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 3, marginTop: 2 }}>
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <span style={{ fontSize: 10, color: "rgba(255,255,255,0.4)" }}>Pause between plays</span>
                            <span style={{ fontSize: 10, color: "rgba(255,255,255,0.6)" }}>
                              {(logo.animGap ?? 2.5) >= 60 ? "1m" : `${(logo.animGap ?? 2.5).toFixed(1)}s`}
                            </span>
                          </div>
                          <input
                            type="range" min={0} max={60} step={0.5}
                            value={logo.animGap ?? 2.5}
                            onChange={(e) => {
                              const updated = (bs.logos ?? []).map((l) => l.id === logo.id ? { ...l, animGap: Number(e.target.value) } : l);
                              localUpdate({ logos: updated });
                              update({ logos: updated });
                            }}
                            style={{ accentColor: "#fb7185", cursor: "pointer" }}
              ...
              <button
                onClick={() => {
                  const newLogo = { id: String(Date.now() + Math.random()), url: "", x: 85, y: 3, size: 120, animation: "Fade" as const, animGap: 2.5 };
                  const updated = [...(bs.logos ?? []), newLogo];
                  localUpdate({ logos: updated });
                  update({ logos: updated });
                }}
                style={{
                  ...
                  background: "rgba(251,113,133,0.05)", color: "#fda4af", transition: "all 0.2s ease",
                }}
              >
                + Add Logo
              </button>

              {/* Go Live / Stop — only shown when at least one logo has an image */}
              {(bs.logos ?? []).some((l) => l.url) && (
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <ToggleButton
                    active={bs.logoActive}
                    accent="#fb7185"
                    countdownSecs={countdowns["logo"]}
                    onCancel={() => cancelGoLive("logo")}
                    onActivate={() => {
                      const firstWithUrl = bs.logos.find((l) => l.url);
                      goLive("logo", {
                        logoActive: true,
                        logos: bs.logos,
                        logoUrl: firstWithUrl?.url ?? "",
                        logoX: firstWithUrl?.x ?? 85,
                        logoY: firstWithUrl?.y ?? 3,
                        logoSize: firstWithUrl?.size ?? 120,
                        logoOpacity: bs.logoOpacity,
                        logoBgEnabled: bs.logoBgEnabled,
                        logoBgColor: bs.logoBgColor,
                        logoBgShape: bs.logoBgShape,
                        logoAnimation: bs.logoAnimation,
                        logoAnimGap: bs.logoAnimGap,
                      });
                    }}
                    onDeactivate={() => stopOverlay({ logoActive: false })}
                  />
                  <LiveBadge label="Logo overlay" active={bs.logoActive} accent="#fb7185" />
                </div>
              )}
              ...
              <div style={{ padding: "9px 14px", borderRadius: 8, background: "rgba(251,113,133,0.06)", border: "1px solid rgba(251,113,133,0.15)", fontSize: 11, color: "rgba(255,255,255,0.4)", lineHeight: 1.6 }}>
                The atmosphere gradient overlays coloured glow blobs on top of the video (useful as an OBS browser source). It is visible in letterbox bars.
              </div>

              {/* ── Video Output Gradient ── */}
              <SectionDivider label="Video Output Gradient" />
              <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, lineHeight: 1.6 }}>
                A solid gradient that fills the entire video output frame — always behind the video. When the stream feed fails or pauses, viewers see the gradient instead of a black screen. Ideal for Facebook, Instagram, and TikTok Live.
              </div>

              {/* Live preview */}
              <div style={{
                borderRadius: 12, overflow: "hidden", height: 72, position: "relative",
                background: `linear-gradient(135deg, ${bs.videoOutputGradient1}, ${bs.videoOutputGradient2})`,
                border: `1px solid ${bs.videoOutputGradientActive ? "rgba(52,211,153,0.6)" : "rgba(255,255,255,0.1)"}`,
                transition: "border-color 0.3s ease",
                boxShadow: bs.videoOutputGradientActive ? `0 0 16px rgba(52,211,153,0.2)` : "none",
              }}>
                {!bs.videoOutputGradientActive && (
                  <div style={{
                    position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center",
                    background: "rgba(0,0,0,0.5)",
                    fontSize: 11, color: "rgba(255,255,255,0.6)", fontWeight: 600, gap: 6,
                  }}>
                    <span style={{ opacity: 0.7 }}>📺</span> Off — enable to set as video background
                  </div>
                )}
                {bs.videoOutputGradientActive && (
                  <div style={{
                    position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 11, color: "rgba(255,255,255,0.9)", fontWeight: 700, gap: 6,
                    textShadow: "0 1px 4px rgba(0,0,0,0.6)",
                  }}>
                    ✓ Active — gradient is live behind video output
                  </div>
                )}
              </div>

              {/* Dedicated colour pickers */}
              <div style={{ display: "flex", gap: 10 }}>
                {(["videoOutputGradient1", "videoOutputGradient2"] as const).map((field, i) => (
                  <div key={field} style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.07em" }}>
                      Colour {i + 1}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <input
                        ...
                        onChange={(e) => {
                          const val = e.target.value;
                          localUpdate({ [field]: val } as any);
                          if ((window as any).__vogDebTimer) clearTimeout((window as any).__vogDebTimer);
                          (window as any).__vogDebTimer = setTimeout(() => update({ [field]: val } as any), 400);
                        }}
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {[
                  { label: "Blue-Green", c1: "#003399", c2: "#00cc44" },
                  { label: "Ocean",      c1: "#0f2027", c2: "#2c5364" },
                  { label: "Midnight",   c1: "#0f0c29", c2: "#302b63" },
                  { label: "Sunset",     c1: "#f7971e", c2: "#c71d6f" },
                  { label: "Forest",     c1: "#134e5e", c2: "#71b280" },
                  ...
                  <button
                    key={preset.label}
                    onClick={() => update({ videoOutputGradient1: preset.c1, videoOutputGradient2: preset.c2 })}
                    style={{
                      display: "flex", alignItems: "center", gap: 6,
                      padding: "4px 10px", borderRadius: 20, cursor: "pointer",
                      border: "1px solid rgba(255,255,255,0.1)",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <button
                  onClick={() => {
                    const next = !bs.videoOutputGradientActive;
                    localUpdate({ videoOutputGradientActive: next });
                    update({
                      videoOutputGradientActive: next,
                      videoOutputGradient1: bs.videoOutputGradient1,
                      videoOutputGradient2: bs.videoOutputGradient2,
                    });
                  }}
                  style={{
                    padding: "8px 20px",
                    borderRadius: 8,
                    border: `1px solid ${bs.videoOutputGradientActive ? "rgba(52,211,153,0.5)" : "rgba(255,255,255,0.15)"}`,
                    background: bs.videoOutputGradientActive
                      ? "rgba(52,211,153,0.15)"
                      : "rgba(255,255,255,0.06)",
                    color: bs.videoOutputGradientActive ? "#34d399" : "rgba(255,255,255,0.7)",
                ...
                >
                  <span style={{
                    ...
                    background: bs.videoOutputGradientActive ? "#34d399" : "rgba(255,255,255,0.2)",
                    display: "inline-block",
                    boxShadow: bs.videoOutputGradientActive ? "0 0 8px #34d399" : "none",
                    transition: "all 0.2s ease",
                  }} />
                  {bs.videoOutputGradientActive ? "Turn Off" : "Turn On"}
                </button>
                <div style={{ fontSize: 12, color: bs.videoOutputGradientActive ? "#34d399" : "rgba(255,255,255,0.35)", fontWeight: 600 }}>
                  Video Output Gradient {bs.videoOutputGradientActive ? "● ON" : "○ OFF"}
                </div>
              </div>

              <div style={{ padding: "9px 14px", borderRadius: 8, background: "rgba(52,211,153,0.05)", border: "1px solid rgba(52,211,153,0.12)", fontSize: 11, color: "rgba(255,255,255,0.4)", lineHeight: 1.6 }}>
                Uses the same colours as the pickers above. When enabled, the gradient becomes the solid background of the video output — perfect for Facebook and Instagram Live where a black fallback screen can interrupt the stream.
              </div>
            </div>
          )}
                ...
                >
                  ...
                  {streams.filter((s: Stream) => s.status === "running" || s.status === "idle").map((s: Stream) => {
                    const label = s.sourceType === "tiktok" ? `@${s.tiktokUsername}`
                      ...
                      : s.sourceType === "link" ? ((s as any).linkSourceUrl || "Video Link")
          ...
          {activeTab === "screen" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {/* iframe / no-API guard — only block when the browser genuinely
                  lacks getDisplayMedia. Modern Android Chrome (111+) DOES
                  support screen capture (it hands off to the OS's screen
                  recorder, briefly showing the home launcher / app chooser
                  before returning "Entire screen" as a capturable surface),
                  so we must not hard-block based on a mobile user-agent
                  check — only iOS Safari and old/embedded browsers truly
                  lack the API, and those fall into the branch below. */}
                ...
                <>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      {!screenActive && !screenConnecting && (
                        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", lineHeight: 1.5 }}>
                          When the browser's share picker opens, choose{" "}
                          <strong style={{ color: "rgba(255,255,255,0.65)" }}>"Entire Screen"</strong>{" "}
                          (not a specific tab or window) to broadcast your whole desktop.
                        </div>
                      )}
                      {screenActive && (
